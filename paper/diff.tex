...
%DIF DEL cav_determinisim_main-original.tex   Sun Feb 14 23:23:43 2021
%DIF ADD cav_determinisim_main.tex            Thu Mar  4 02:39:45 2021
...
%
Thus, \DIFaddbegin \DIFadd{in a deterministic simulation environment, }\DIFaddend tests are rendered repeatable and yield simulation results that are trustworthy and straightforward to debug. 
%
...
%
\DIFdelbegin \DIFdel{This is then followed by a }\DIFdelend \DIFaddbegin \DIFadd{A }\DIFaddend case study using CARLA, an open-source autonomous driving simulation environment powered by Unreal Engine, \DIFdelbegin \DIFdel{which highlights }\DIFdelend \DIFaddbegin \DIFadd{is then presented to highlight }\DIFaddend its inherent shortcomings in providing sufficient precision in experimental results. 
%
Different configurations and utilisations of the software and hardware are explored to determine an operational domain where the simulation precision is \DIFdelbegin \DIFdel{satisfactory }\DIFdelend \DIFaddbegin \DIFadd{sufficiently }\DIFaddend low i.e.\DIFdelbegin \DIFdel{variance between repeats }\DIFdelend \DIFaddbegin \DIFadd{\ variance between repeated executions }\DIFaddend becomes negligible for development and testing work.  
...
\DIFaddbegin {\color{red} \sc \DIFadd{Ensure past/present is used consistently and appropriately throughout.}}
...
\DIFdelbegin \DIFdel{A list of definitions are given here which }\DIFdelend \DIFaddbegin \DIFadd{Several definitions are introduced in this section. These }\DIFaddend are used in the subsequent discussion. Refer to Fig.~\ref{variance_description} throughout this section.
\\
...
A simulation can be thought of as the \DIFdelbegin \DIFdel{generation or production of }\DIFdelend \DIFaddbegin \DIFadd{process of generating or producing }\DIFaddend experimental data. 
%
...
%
If a simulation is deterministic\DIFaddbegin \DIFadd{, Fig.~\ref{variance_description}~(b), }\DIFaddend then there will be no variation in the generated output data, i.e. all future states are perfectly reproducible from prior states and actions. 
%
However, if a simulation is non-deterministic\DIFaddbegin \DIFadd{, Fig.~\ref{variance_description}~(a), }\DIFaddend then there will be a variation in the generated output data. \\
...
We adopt \DIFdelbegin \DIFdel{some terms }\DIFdelend \DIFaddbegin \DIFadd{terminology }\DIFaddend from the mechanical engineering and statistics \DIFdelbegin \DIFdel{domain that best }\DIFdelend \DIFaddbegin \DIFadd{domains to }\DIFaddend describe when there is variation in the generated output data~\cite{ADictionaryofMechanicalEngineering}.
%
...
%
Therefore, variance can indicate the degree to which a simulation can repeatedly generate the same result when executed \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{under }\DIFaddend the same conditions and actions\DIFdelbegin \DIFdel{taken
...
As an analogy, the simulator can be thought of as a manufacturing process \DIFdelbegin \DIFdel{, producing dataand to find the value of the precision then the }\DIFdelend \DIFaddbegin \DIFadd{that produces data. To determine the precision of this process, the }\DIFaddend output must be measured \DIFaddbegin \DIFadd{and analysed }\DIFaddend for differences when the process is repeated. 
%DIF > 
Those differences describe the spread or variance in the process output\DIFdelbegin \DIFdel{and a }\DIFdelend \DIFaddbegin \DIFadd{. A }\DIFaddend hard limit on the variance can \DIFaddbegin \DIFadd{then }\DIFaddend be defined, Fig.~\ref{variance_description}\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{~(a)}\DIFaddend , beyond which the output fails to meet the required tolerance, e.g.\DIFdelbegin \DIFdel{rejected for }\DIFdelend \DIFaddbegin \DIFadd{\ the output is rejected by }\DIFaddend quality control.
%
...
%
Real manufacturing fails to achieve absolute precision\DIFdelbegin \DIFdel{and hence the }\DIFdelend \DIFaddbegin \DIFadd{. Hence, there is a }\DIFaddend need for tolerances \DIFdelbegin \DIFdel{on design specifications to }\DIFdelend \DIFaddbegin \DIFadd{to be specified to }\DIFaddend account for the variance in real-world \DIFaddbegin \DIFadd{manufacturing }\DIFaddend processes. 

If a simulator is deterministic then it will produce results with absolute precision or zero variance, Fig.~\ref{variance_description}\DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{~(b)}\DIFaddend , and hence will be within \DIFdelbegin \DIFdel{acceptable tolerance }\DIFdelend \DIFaddbegin \DIFadd{tolerance by design}\DIFaddend . If the simulator is non-deterministic then there will be a \DIFaddbegin \DIFadd{measurable, non-zero }\DIFaddend variance in the output \DIFdelbegin \DIFdel{which can be measured}\DIFdelend \DIFaddbegin \DIFadd{data}\DIFaddend .\\
%

\subsubsection{Accuracy}
Precision and tolerance should not to be confused with \textit{accuracy}\DIFaddbegin \DIFadd{, }\DIFaddend which describes how closely the mean of the generated output of a process aligns to a known standard or reference value. We therefore define accuracy as the difference between the true value, or reference value, and what has been achieved in the \DIFdelbegin \DIFdel{simulation or generation process }\DIFdelend \DIFaddbegin \DIFadd{data generation process or simulation}\DIFaddend . 
%
...
\DIFdelbegin \DIFdel{In most cases such accuracy will not be possible due to }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{In practice full accuracy will often not be achievable due to modelling and associated }\DIFaddend computational demands of \DIFdelbegin \DIFdel{such an exact replicaand in most casesis unnecessary, where }\DIFdelend \DIFaddbegin \DIFadd{creating and executing exact replica. In most cases, in fact, it is unnecessary and }\DIFaddend some authors state that `just the right amount of realism' is required to achieve valid simulation results~\cite{Koopman2018}. \\

\subsubsection{Simulation Trace}
A simulation trace is the output log from the simulator consisting of a time series of all actor positions ($x,y,z$) \DIFaddbegin \DIFadd{in a 3D environment recorded }\DIFaddend at regular time intervals. This definition could be extended to include other variables. %, but position is sufficient for this analysis.\\
A set of simulation traces derived from the same input \DIFaddbegin \DIFadd{and starting state }\DIFaddend then forms the experimental data on which variance is calculated \DIFaddbegin \DIFadd{for a given simulation run}\DIFaddend . \\


\subsubsection{Simulation Variance \& Deviation}
If the simulator is non-deterministic then how can \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{the simulation }\DIFaddend variance be measured? This can be achieved by monitoring \DIFdelbegin \DIFdel{any simulated }\DIFdelend \DIFaddbegin \DIFadd{the values of any of the recorded }\DIFaddend output variables that should be consistent from run to run. 
...
\DIFdelbegin \DIFdel{Actor path variance, derived from the simulation trace, is chosen over other variables as this distance based metric forms the basis for many downstream verification tests. 
%DIF <  such as assertion checking. 
}\DIFdelend \DIFaddbegin \DIFadd{For example, actor position variance is a distance-based metric that can be derived from simulation traces. The actor position over time, i.e.\ the actor path, is often used in assertion checking, e.g.\ to determine whether vehicles keep within lanes or whether minimum distances to other vehicles and road users are being maintained. 
}\DIFaddend % 
\DIFdelbegin \DIFdel{Therefore the }\DIFdelend \DIFaddbegin \DIFadd{Thus, in the case study presented in this paper, the }\DIFaddend term \textit{simulation variance}\DIFaddbegin \DIFadd{, measured in SI unit $m^2$, }\DIFaddend refers to a measure of actor position variance in the simulation with respect to time\DIFaddbegin \DIFadd{, }\DIFaddend assuming fixed actions. \DIFaddbegin {\color{red} \DIFadd{Unclear: Why wrt.\ time, that would then be the actor path, or not?}}
\DIFaddend %
\DIFdelbegin \textit{\DIFdel{Deviation}} %DIFAUXCMD
\DIFdelend %DIF > \textit{Deviation} (SI unit $m$) is the square root of variance (SI unit $m^2$), which is a more intuitive measure to comprehend when interpreting test results.
%DIF > 
\DIFaddbegin \DIFadd{Case study results are presented using deviation }\DIFaddend (SI unit $m$)\DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the square root of variance\DIFdelbegin \DIFdel{(SI unit $m^2$), which }\DIFdelend \DIFaddbegin \DIFadd{, rather than variance, as this }\DIFaddend is a more intuitive \DIFdelbegin \DIFdel{value }\DIFdelend \DIFaddbegin \DIFadd{measure }\DIFaddend to comprehend when \DIFdelbegin \DIFdel{considering interpretation of verification tests.
%DIF < Results are referred to in terms of deviation, the square root of variance, rather than variance, as this is more intuitive to comprehend when considering verification tests, e.g. vehicle close-passing. Moreover, the maximum observed value of deviation was chosen over a mean value to ensure sufficient sensitivity.\\
}\DIFdelend \DIFaddbegin \DIFadd{interpreting test results.
%DIF > 
%DIF >  more intuitive to comprehend when considering verification tests, e.g. vehicle close-passing. Moreover, the maximum observed value of deviation was chosen over a mean value to ensure sufficient sensitivity.\\
}\DIFaddend \\

\subsubsection{Scene, Scenario \& Situation}
We \DIFdelbegin \DIFdel{will }\DIFdelend adopt the terminology defined \DIFaddbegin \DIFadd{for automated driving }\DIFaddend in~\cite{Ulbrich2015}, where \textit{scene} refers to all static objects including the road network, street furniture, environment conditions and a snapshot of any dynamic elements. Dynamic elements are the elements in a scene whose actions or behaviour may change over time; these are considered actors and may include the AV\DIFaddbegin \DIFadd{, }\DIFaddend or \textit{ego vehicle}, other road vehicles, cyclists, pedestrians and traffic signals. The \textit{scenario} is then defined as a temporal development between several scenes which may be specified by specific parameters. A \textit{situation} is defined as the subjective conditions and determinants for behaviour at a particular point in time.
...
Determinism is a key \DIFdelbegin \DIFdel{prerequisite }\DIFdelend \DIFaddbegin \DIFadd{requirement }\DIFaddend for simulation during AV development and testing. A deterministic simulation environment guarantees that tests are repeatable\DIFdelbegin \DIFdel{. A deterministic simulator can be considered to have }\DIFdelend \DIFaddbegin \DIFadd{, i.e.\ repeated runs of a test, given the same initial conditions and event history, produce the same output data.
%DIF > 
Thus, a deterministic simulator has }\DIFaddend zero \textit{variance}. 
\DIFdelbegin \DIFdel{If the variance is }\DIFdelend \DIFaddbegin 

\DIFadd{A simulator with }\DIFaddend non-zero \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{variance }\DIFaddend can no longer be considered deterministic\DIFdelbegin \DIFdel{but this }\DIFdelend \DIFaddbegin \DIFadd{. 
%DIF > 
Non-deterministic simulators }\DIFaddend may be sufficient for certain applications as long as \DIFdelbegin \DIFdel{it is }\DIFdelend \DIFaddbegin \DIFadd{their variance is permissible, i.e.\ }\DIFaddend \textit{within tolerance}. 
%DIF > 
Therefore, \textit{tolerance} is the acceptable degree of variability between repeated \DIFdelbegin \DIFdel{simulation traces}\DIFdelend \DIFaddbegin \DIFadd{simulations}\DIFaddend . When the simulation output is within tolerance, coverage results are stable and, when a test fails, debugging can rely on the test producing the same trace and outcome when repeated. 
%DIF > 
...
If the simulation is non-deterministic\DIFdelbegin \DIFdel{and has a non-permissible }\DIFdelend \DIFaddbegin \DIFadd{, e.g.\ 
%DIF >  KIE No, whether the variance is or is not permissible is not what matters for non-det, it should be zero!
%DIF > e.g. it has a non-permissible variance in,
it has a non-zero }\DIFaddend variance in, for example, actor positions, \DIFdelbegin \DIFdel{this may lead to assessment errors}\DIFdelend \DIFaddbegin \DIFadd{then this may, in the best case, lead to intermittent assertion failures}\DIFaddend , making it difficult to \DIFaddbegin \DIFadd{re-produce, }\DIFaddend understand and remove bugs \DIFaddbegin \DIFadd{and rendering verification results unstable}\DIFaddend . In the worst case, \DIFaddbegin \DIFadd{however, }\DIFaddend bugs that could have been identified in simulation remain undetected, \DIFdelbegin \DIFdel{resulting in }\DIFdelend \DIFaddbegin \DIFadd{leading to }\DIFaddend false confidence in the safety of the AV's control software. 
%
...
% Challenges of SBV
When used for gaming, game engines do not need to be deterministic nor do they \DIFdelbegin \DIFdel{even }\DIFdelend have any requirements on the limits of permissible variance; there are no safety implications from non-determinism in this domain, nor is finding and fixing all the bugs \DIFdelbegin \DIFdel{related to non-determinism }\DIFdelend %DIF >  related to non-determinism 
a high priority for games developers. It could even be argued that simulation variance is a feature that enhances gaming and improves the user experience. However, the situation is very different for AV development and testing. Thus, our main research \DIFdelbegin \DIFdel{question is}\DIFdelend \DIFaddbegin \DIFadd{questions are}\DIFaddend :
%
\todo[inline,color=red!40]{old: How can one assess the extent to which a simulation environment is deterministic?  }
%
\DIFaddbegin {\em \DIFaddend How can one assess \DIFdelbegin \DIFdel{the extent to which }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend a simulation environment is deterministic\DIFdelbegin \DIFdel{or has a permissible level of variance?}\DIFdelend \DIFaddbegin \DIFadd{?}} \DIFadd{and 
}{\em \DIFadd{How can one determine and control the simulation variance?}}
\DIFaddend \todo[inline,color=green!40]{GC Alternative: How can one assess the extent to which a simulation environment is deterministic or has a permissible tolerance?} 

% In pursuit of an answer to this question we
In this paper we \DIFdelbegin \DIFdel{investigated how the non-determinism of Carla, }\DIFdelend \DIFaddbegin \DIFadd{investigate non-determinism and how it affects simulation results on the example of CARLA, an open-source autonomous driving simulation environment }\DIFaddend based on the Unreal game engine\DIFdelbegin \DIFdel{, when used for simulation-based AV verification affects the simulation results}\DIFdelend .
%
In our case study, scenarios between pedestrian and vehicle actors \DIFdelbegin \DIFdel{have been analysed to identify conditions that result in non-deterministic simulation output by analysis of }\DIFdelend \DIFaddbegin \DIFadd{are investigated to determine the }\DIFaddend actor position variance \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{in the simulation output for repeated simulation runs. 
%DIF >  KIE: This is covered in the next sentence:
%DIF >  to identify conditions that result in non-determinism based on actor position variance in the simulation output.
}\DIFaddend %
...
%
\DIFdelbegin \DIFdel{By analysing actor position variance we }\DIFdelend %DIF >  By analysing actor position variance 
\DIFaddbegin \DIFadd{We }\DIFaddend find that the \DIFdelbegin \DIFdel{Carla }\DIFdelend \DIFaddbegin \DIFadd{CARLA }\DIFaddend simulator is non-deterministic under certain conditions. \DIFaddbegin {\color{red} \DIFadd{Say what the observed variance range is, and what we consider permissible for this study.}}
\DIFaddend % However, by limiting the level of system utilisation to less than 75\% and terminating scenarios once a collision has been detected, a permissible level of simulation variance was observed.
%
\DIFdelbegin \DIFdel{Carla }\DIFdelend \DIFaddbegin \DIFadd{CARLA }\DIFaddend exhibits a permissible \DIFdelbegin \DIFdel{level of }\DIFdelend variance when system utilisation is restricted to 75\% or less and \DIFdelbegin \DIFdel{ensuring termination of scenarios }\DIFdelend \DIFaddbegin \DIFadd{the simulation is terminated }\DIFaddend once a vehicle collision has been detected. \DIFaddbegin {\color{red} \DIFadd{Why ``vehicle'' collision and not just ``collision''?}}
\DIFaddend %
...
%DIF > impact that using a game engine for AV simulation may have.
%DIF >  on verification tasks. 
...
In exceptional cases\DIFaddbegin \DIFadd{, }\DIFaddend where computational resources are scarce, the fixed time step can be greater than the time between render ticks %it takes to process an ``Update" inner loop on some (slow) machines, 
...
%
Considering the objectives for gaming and comparing them to these for AV development and testing, there are fundamental differences. Providing game players with a responsive real-time experience is often achieved at the cost of simulation accuracy and precision. The gamer neither \DIFdelbegin \DIFdel{wishes }\DIFdelend \DIFaddbegin \DIFadd{needs }\DIFaddend a faithful representation of reality, i.e.\DIFaddbegin \DIFadd{\ }\DIFaddend the gamer will accept low \textit{accuracy}, nor do they require for repeated actions to result in the same outcome to within a particular tolerance\DIFdelbegin \DIFdel{, i. e. can be low }\textit{\DIFdel{precision}}%DIFAUXCMD
\DIFdelend . %DIF > , i.e.\ can be low \textit{precision}.
%
In contrast, the precision required for AV development and testing is \DIFdelbegin \DIFdel{much higher}\DIFdelend \DIFaddbegin \DIFadd{very high}\DIFaddend , especially for perception stack testing, but also\DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{, more generally, for obtaining reliable verification results and for }\DIFaddend visual inspection of accidents or near misses. If necessary, it is acceptable to achieve the required \DIFdelbegin \DIFdel{level of tolerance on the precision }\DIFdelend \DIFaddbegin \DIFadd{tolerance %DIF > on the precision 
}\DIFaddend at the cost of real-time performance. For example, for perception stack testing the sensors need to get input that is repeatable so that if any software bugs are found they can be re-played and the issue resolved. This  may only be realisable by slowing down the rendering to enable more extensive physics calculations.
%
...
%DIF < 
%DIF <  SEE BELOW \todo[inline]{Check the points on floating point operations in An Empirical Analysis of Flaky Tests~\cite{empirical-analysis-of-flaky-tests}.}
\DIFaddbegin 

%DIF >  SEE BELOW \todo[inline]{Check the points on floating-point operations in An Empirical Analysis of Flaky Tests~\cite{empirical-analysis-of-flaky-tests}.}
{\color{red} \DIFadd{PROBLEMATIC: }\DIFaddend Even operations such as the average of an array can result in issues with overflows \DIFdelbegin \DIFdel{, underflows and }\DIFdelend \DIFaddbegin \DIFadd{and underflows }{\sc \DIFadd{(these can happen with Integers too)}}
\DIFadd{and,  }\DIFaddend non-associative addition~\cite{Kapre2007} \DIFaddbegin {\sc \DIFadd{(only when the exec order is changed)}} \DIFaddend which may result in non-deterministic behaviour. \DIFdelbegin \DIFdel{Calculation resultsmay differ }\DIFdelend \DIFaddbegin }

%DIF >  As a consequence, floating-point arithmetic is not associative. Thus, for arithmetic expressions beyond two operands, such as $x+y+z$, a change of execution order, e.g.\ from $x+(y+z)$ to $(x+y)+z$, can lead to different results~\cite{Kapre2007}.
%DIF > 
\DIFadd{As a consequence, floating-point arithmetic is not associative. Thus, for arithmetic expressions beyond two operands, such as $x+y+z$, executing $x+(y+z)$ rather than $(x+y)+z$, can produce different results~\mbox{%DIFAUXCMD
\cite{Kapre2007}}%DIFAUXCMD
. This is because rounding is being performed at different stages in the computation, depending on the respective execution order. 
%DIF > 
Execution order changes can occur }\DIFaddend due to the \DIFdelbegin \DIFdel{nature of non-associative floating point arithmetic if there is a change in compiler , a change in the execution order or even }\DIFdelend \DIFaddbegin \DIFadd{use of a different compiler or as a consequence of parallelisation, e.g.\ within the runtime environment or at the hardware level. 
%DIF > 
Furthermore, floating-point calculation results may differ }\DIFaddend if the execution is performed on a GPU rather than a CPU which may have different register widths~\cite{Whitehead2011}. 
\DIFdelbegin \DIFdel{Some authors even suggest avoiding the use of floating point entirely for assertion testing due to imprecision and non-deterministic behaviour~\mbox{%DIFAUXCMD
\cite{empirical-analysis-of-flaky-tests}}%DIFAUXCMD
. 
}\DIFdelend %
%DIF > {\color{red} Some authors even suggest avoiding the use of floating-point entirely for assertion testing due to imprecision and non-deterministic behaviour~\cite{empirical-analysis-of-flaky-tests}.}
%DIF > 
%If calculations are performed on a single core, then concurrency of multiple threads is not an issue.
\DIFdelbegin %DIFDELCMD < \todo[inline,color=green!40]{GC Addition (added reference on non-associative fp addition Kapre2007, Whitehead2011, empirical-analysis-of-flaky-tests, see para below:)
%DIFDELCMD < 

%DIFDELCMD < Even operations such as the average of an array can result in issues with overflows, underflows and non-associative addition~\cite{Kapre2007} which may result in non-deterministic behaviour. Calculation results may differ due to the nature of non-associative floating point arithmetic if there is a change in compiler, a change in the execution order or even if the execution is performed on a GPU rather than a CPU which may have different register widths~\cite{Whitehead2011}. Some authors even suggest avoiding the use of floating point entirely for assertion testing due to imprecision and non-deterministic behaviour~\cite{empirical-analysis-of-flaky-tests}. 
%DIFDELCMD < %
%DIFDELCMD < If calculations are perofrmed on a single core, then concurrency of multiple threads is not an issue.
%DIFDELCMD < % which is discussed in the section below titled Scheduling, Concurrency and Parallelisation.}
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \todo[inline,color=green!40]{GC Addition (added reference on non-associative fp addition Kapre2007, Whitehead2011, empirical-analysis-of-flaky-tests, see para below:)
...
%
Thus, one could \DIFdelbegin \DIFdel{jump to the conclusion }\DIFdelend \DIFaddbegin \DIFadd{conclude }\DIFaddend that floating-point \DIFdelbegin \DIFdel{errors cause non-deterministic behaviour}\DIFdelend \DIFaddbegin \DIFadd{arithmetic causes non-determinism}\DIFaddend , resulting in loss of repeatability. \DIFaddbegin \DIFadd{In fact, some authors suggest avoiding the use of floating-point representation entirely for assertion testing due to imprecision and non-deterministic behaviour~\mbox{%DIFAUXCMD
\cite{empirical-analysis-of-flaky-tests}}%DIFAUXCMD
. 
}\DIFaddend 


\DIFdelbegin \DIFdel{However, these precision issues }\DIFdelend \DIFaddbegin \DIFadd{In contrast, we argue that the precision issues related to floating-point operations }\DIFaddend are better described as \textit{incorrectness} \DIFdelbegin \DIFdel{, rather than them being a source of }\DIFdelend \DIFaddbegin \DIFadd{that is in fact repeatable; they do not cause }\DIFaddend non-determinism \DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{per se. }{\color{red} \DIFadd{Should this be IMPRECISION rather than incorrectness?}}
\DIFaddend %
It is normally reasonable to assume that the processors on which game engines run are \DIFdelbegin \DIFdel{designed to be }\DIFdelend deterministic with respect to floating-point arithmetic. 
%
Thus, \DIFdelbegin \DIFdel{when a rounding error occurs, then }\DIFdelend \DIFaddbegin \DIFadd{given the same operands and sequence of operations, }\DIFaddend the same processor should \DIFaddbegin \DIFadd{always }\DIFaddend produce the same \DIFdelbegin \textit{\DIFdel{incorrect}} %DIFAUXCMD
\DIFdel{result given the same operands. }\DIFdelend \DIFaddbegin \DIFadd{bit pattern. %DIF > , i.e.\ a result that is as consistently equally \textit{incorrect} over a number of executions.
}\DIFaddend %
So, even if the result of a floating-point operation is incorrect due to floating-point rounding errors, it should always be equally \textit{incorrect} \DIFdelbegin \DIFdel{as long as the implementation conforms }\DIFdelend \DIFaddbegin \DIFadd{for implementations that conform }\DIFaddend to the IEEE floating-point standard~\cite{8766229}.

\DIFdelbegin \DIFdel{Beyond hardware limitations, }\DIFdelend \DIFaddbegin \DIFadd{However, as illustrated above on the example of non-associative addition, a change of execution order can lead to sequences of floating point operations producing different results. Thus, any uncontrolled options to modify execution order, e.g.\ at the compiler level, within the runtime environment or at the hardware level, can cause simulation results to differ between runs, resulting in non-zero variance and loss of determinism. 
 }

\DIFadd{Beyond that, }\DIFaddend aggressive optimisations by the compiler can also introduce incorrectness in floating-point arithmetic~\cite{llvm-floating-point}. However, the same executable should still return the same output for identical input. 
%DIF < 
\DIFdelbegin \DIFdel{Therefore, it can be concluded that }\DIFdelend \DIFaddbegin 

\DIFadd{In conclusion, }\DIFaddend floating-point arithmetic does not \DIFdelbegin \DIFdel{contribute to }\DIFdelend \DIFaddbegin \DIFadd{cause non-zero }\DIFaddend \textit{simulation variance} for repeated \DIFdelbegin \DIFdel{tests }\DIFdelend \DIFaddbegin \DIFadd{simulation runs when }\DIFaddend using the same executable on the same hardware with \DIFaddbegin \DIFadd{exactly }\DIFaddend the same configuration \DIFaddbegin \DIFadd{and execution order}\DIFaddend .
...
%
\DIFdelbegin \DIFdel{Changing }\DIFdelend \DIFaddbegin \DIFadd{In principle, changing }\DIFaddend the scheduling policy and thread priorities may increase simulation variance. 
...
Indeed, the developers of the \DIFdelbegin \DIFdel{de-bugging program }\DIFdelend \DIFaddbegin \DIFadd{debugging program }\DIFaddend \texttt{\DIFdelbegin \DIFdel{RR}\DIFdelend \DIFaddbegin \DIFadd{rr}\DIFaddend }~\cite{RR_link} took significant steps to ensure deterministic behaviour of their program by executing or context-switching all processes to a single core, which avoids data races as single threads cannot concurrently access shared memory. This allowed control over the scheduling and execution order of threads promoting deterministic behaviour \DIFaddbegin \DIFadd{by design}\DIFaddend ~\cite{acm-q-rr-interview}.
%
\DIFdelbegin %DIFDELCMD < \todo[inline,color=green!40]{GC addition:Indeed, the developers of the de-bugging program \texttt{RR} took significant steps to ensure deterministic behaviour of their program by executing or context-switching all processes to a single core, which avoids data races as single threads cannot concurrently access shared memory. This allowed control over the scheduling and execution order of threads promoting deterministic behaviour~\cite{acm-q-rr-interview}. }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \todo[inline,color=green!40]{GC addition:Indeed, the developers of the de-bugging program \texttt{rr} took significant steps to ensure deterministic behaviour of their program by executing or context-switching all processes to a single core, which avoids data races as single threads cannot concurrently access shared memory. This allowed control over the scheduling and execution order of threads promoting deterministic behaviour~\cite{acm-q-rr-interview}. }
\DIFaddend 
...
%DIF <  i.e. the utilisation of hardware is manipulated differently each time. This would then result in non-deterministic behaviours especially if it is combined with the floating point problem discussed earlier. 
%DIF >  i.e. the utilisation of hardware is manipulated differently each time. This would then result in non-deterministic behaviours especially if it is combined with the floating-point problem discussed earlier. 
%
...
%
\DIFdelbegin %DIFDELCMD < \todo[inline,color=red!40]{old: To ensure repeatable simulation outcomes the physics setting $dt$ must be fixed, along with any actor navigation meshes, random number seeds, game engine setup and simulation specific parameters.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \todo[inline,color=red!40]{old: To ensure repeatable simulation outcomes the physics setting $dt$ must be fixed, along with any actor navigation meshes, seeds for random number generation, game engine setup and simulation specific parameters.}
\DIFaddend To ensure precise simulation outcomes the physics setting\DIFaddbegin \DIFadd{, }\DIFaddend $dt$\DIFaddbegin \DIFadd{, }\DIFaddend must be fixed, along with any actor navigation meshes, \DIFdelbegin \DIFdel{random number seeds}\DIFdelend \DIFaddbegin \DIFadd{seeds for random number generation}\DIFaddend , game engine setup and simulation specific parameters.
...
%
\DIFdelbegin \DIFdel{NUMA }\DIFdelend \DIFaddbegin \DIFadd{Non-uniform memory access (NUMA) }\DIFaddend should only affect interprocessor data transfer cost and\DIFaddbegin \DIFadd{, }\DIFaddend without control measures\DIFaddbegin \DIFadd{, }\DIFaddend will only make the computation cycle longer. Relative access times between different caches are likely to be small although may have a more pronounced impact on high throughput systems, e.g.\ HPC. \DIFaddbegin {\color{red} \DIFadd{Not sure this is correct, see the NUMA section above. Why ``only''? If one computation out of many takes longer, then their exec order can be affected, or not?}}
\DIFaddend %
...
\bottomrule
\end{tabular}
\caption{A description of the test scenarios showing the test number, the actors included, \DIFdelbeginFL \DIFdelFL{if }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{whether }\DIFaddendFL a collision occurred and \DIFaddbeginFL \DIFaddFL{if so then }\DIFaddendFL between which actors. \DIFdelbeginFL \DIFdelFL{Where }\DIFdelendFL $n$\DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL the number of repeats \DIFaddbeginFL \DIFaddFL{is set to 1000 }\DIFaddendFL and $\max\sigma$ is the maximum simulation deviation. The term \textit{unrestricted} refers to an unrestricted account of the results including results of any resource utilisation. To understand the impact of collisions and high resource utilisation, the \textit{restricted} column shows a subset of the results where post-collision data and experiments above 75\% resource utilisation have been removed.}
...
%
Ultimately, our objective is to control \DIFdelbegin \DIFdel{non-determinisim }\DIFdelend \DIFaddbegin \DIFadd{non-determinism }\DIFaddend to minimise simulation variance.
...
%
We first describe the context, scene and scenario of interest before discussing and defining a tolerance for what is considered an acceptable simulation variance in this context. \DIFaddbegin {\color{red} \DIFadd{and then..? Please cover the entire section - consider splitting setup from evaluation.}}
\DIFaddend %
...
%
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend tolerance must be appropriate to allow accurate assertion checking and coverage collection in the simulation environment, but not so small as to fail with minor computational perturbations. \DIFaddbegin {\color{red} {\sc \DIFadd{What would fail?}}}
\DIFaddend %
...
Therefore, for this case study and\DIFdelbegin \DIFdel{more broadly }\DIFdelend \DIFaddbegin \DIFadd{, more broadly, for }\DIFaddend our general verification requirements, a tolerance of $1$cm has been selected. 
...
\DIFdelbegin \DIFdel{Case }\DIFdelend \DIFaddbegin \DIFadd{In the following, case }\DIFaddend study results are shown in terms of the maximum deviation, $\max\sigma$, from the mean actor path over the entire simulation history where any value higher than the specified tolerance is considered non-permissible.
%
...
Previous investigations into the Unreal Engine indicated that collisions between dynamic actors \DIFaddbegin {\color{red} \DIFadd{Why DYNAMIC?}} \DIFaddend and solid objects, termed \textit{blocking physics bodies} in Unreal Engine documentation~\cite{collision_overview}, can lead to high simulation variance\DIFdelbegin \DIFdel{, \mbox{%DIFAUXCMD
\cite{TSLUnrealEngineTesting}}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{TSLUnrealEngineTesting}}%DIFAUXCMD
. }{\color{red} \DIFadd{NOT ACCESSIBLE}}
\DIFaddend %
Collisions and the subsequent physics calculations that are processed, termed \textit{event hit} callback in Unreal Engine, were \DIFdelbegin \DIFdel{seen }\DIFdelend \DIFaddbegin \DIFadd{identified }\DIFaddend as potentially key aspects to the investigation into simulation variance.

The tests \DIFaddbegin \DIFadd{used for the case study }\DIFaddend are listed in Table~\ref{TableOfExperiments}\DIFdelbegin \DIFdel{and were chosen to }\DIFdelend \DIFaddbegin \DIFadd{. They }\DIFaddend cover a range of interactions between actor types. 
%
Tests 1 \& 2 involve \DIFdelbegin \DIFdel{2 }\DIFdelend \DIFaddbegin \DIFadd{two }\DIFaddend vehicles meeting at a junction where they \DIFaddbegin \DIFadd{either }\DIFaddend do not collide (\DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{Test }\DIFaddend 1) and \DIFdelbegin \DIFdel{when they do tiggering }\DIFdelend \DIFaddbegin \DIFadd{where they do collide (Test 2), thereby triggering }\DIFaddend an \textit{event hit} callback in the game engine\DIFdelbegin \DIFdel{(test 2)}\DIFdelend . In both cases the trajectories of the vehicles are hard-coded to follow a set of waypoints spaced at $0.1$m 
...
\DIFdelbegin \DIFdel{In test }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{In Test }\DIFaddend 3 a mixture of different actor types \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend introduced where two vehicles drive without collision and a pedestrian walks across the road at a crossing point. 
\DIFdelbegin \DIFdel{In test }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{In Test }\DIFaddend 4 this pedestrian collides with one of the vehicles at the crossing, triggering \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend \textit{event hit} callback, see Fig~\ref{Test_a}. Similar to vehicles, pedestrians navigate via a set of regularly spaced waypoints at $0.1$m intervals using the A* algorithm which is the default controller for the CARLA pedestrian actors. Tests 5 \& 6 involve only pedestrians that \DIFaddbegin \DIFadd{either }\DIFaddend do not collide (\DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{Test }\DIFaddend 5) and that do collide (\DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{Test }\DIFaddend 6), see Fig.~\ref{Test_b}. 
...
% ***************************************************
\subsection{\DIFdelbegin \DIFdel{Experiment Description}\DIFdelend \DIFaddbegin \DIFadd{Evaluation Metric}\DIFaddend }\label{s:Experiment_Description}
...
For each test the position of each actor \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend logged at $0.1s$ intervals providing a trace of that actor's \DIFdelbegin \DIFdel{trajectory }\DIFdelend \DIFaddbegin \DIFadd{path }\DIFaddend with respect to simulation time. 
%
\DIFdelbegin \DIFdel{Repeated traces }\DIFdelend \DIFaddbegin \DIFadd{The logs from repeated tests }\DIFaddend are compared at the same time index $t$ for \DIFaddbegin \DIFadd{each }\DIFaddend actor $a$ to provide a value for the variance \DIFaddbegin \DIFadd{of actor $a$ at time index $t$}\DIFaddend , $\sigma_a^{2}(t)$\DIFaddbegin \DIFadd{, at each point in time. }{\color{red} \DIFadd{This gives a variance function over time for each actor.}}

{\color{blue} \DIFadd{The logs from repeated tests are sourced to establish a value for the variance associated with each actor, $a$, at each time point $t$, giving a variance function over time for each actor, $\sigma_a^{2}(t)$}\DIFaddend .%DIF < 
\DIFdelbegin \DIFdel{Herein the }\DIFdelend \DIFaddbegin }

\DIFadd{Instead of using ..., }{\color{red} \sc \DIFadd{please complete}} \DIFadd{herein the }\DIFaddend results are given in terms of the deviation, $\sigma_a(t)$, which indicates the dispersion of the actor position data relative to the mean and is helpfully in the same units as actor position, \DIFaddbegin \DIFadd{i.e.\ metres (}\DIFaddend $m$\DIFaddbegin \DIFadd{)}\DIFaddend , for ease of interpretation.
\DIFdelbegin %DIFDELCMD < \todo[inline,color=green!40]{GC Addition: For each test the position of each actor was logged at $0.1s$ intervals providing a trace of that actor's trajectory with respect to simulation time. 
%DIFDELCMD < %
%DIFDELCMD < Repeated traces are compared at the same time index $t$ for actor $a$ to provide a value for the variance, $\sigma_a^{2}(t)$. 
%DIFDELCMD < %
%DIFDELCMD < Herein the results are given in terms of the deviation, $\sigma_a(t)$, which indicates the dispersion of the actor position data relative to the mean and is helpfully in the same units as actor position, $m$, for ease of interpretation. }
%DIFDELCMD < %%%
\DIFdelend %
\DIFaddbegin \todo[inline,color=green!40]{GC Addition: For each test the position of each actor was logged at $0.1s$ intervals providing a trace of that actor's trajectory with respect to simulation time. 
%
...
\DIFaddend The maximum variance over the entire set of $n$ \DIFdelbegin \DIFdel{simulations is therefore }\DIFdelend \DIFaddbegin \DIFadd{repeated simulations, i.e.\ the overall observed worst case, is }\DIFaddend defined as the largest variance of any actor at any time in \DIFdelbegin \DIFdel{the simulation, 
}\DIFdelend \DIFaddbegin \DIFadd{any of the simulation runs, as given in Equation~\ref{eq:max_sigma}. 
%DIF > {\color{red} This is still a function over time - should it be?}
}\DIFaddend \begin{equation} \label{eq:max_sigma}
\max_{a,t}\sigma_a^{2}(t)
\DIFdelbegin \DIFdel{, 
}\DIFdelend \end{equation}
\DIFdelbegin \DIFdel{and therefore the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend maximum deviation is \DIFdelbegin \DIFdel{simply }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend square root of the maximum variance \DIFaddbegin {\color{red} \DIFadd{The absolute value of the square root?}} \DIFaddend and herein referred to as ${\max\sigma}$ for brevity. \DIFaddbegin {\color{red} \DIFadd{So, ``a'' is any actor or a specific actor? Likewise, ``t'' is any time point or a specific time point? NOTE that any actor any time means a in A and t in T, where A is the set of actors and T is the set of all time points.}}
\DIFaddend 

The maximum deviation, $\max\sigma$, \DIFdelbegin \DIFdel{was analysed against }\DIFdelend \DIFaddbegin \DIFadd{can be analysed for }\DIFaddend the different scenarios and settings that were identified as potential sources of non-determinism, and compared against the limit of \textit{permissible variance} to indicate if the simulation \DIFdelbegin \DIFdel{was reliable }\DIFdelend \DIFaddbegin \DIFadd{is sufficiently accurate }\DIFaddend for verification purposes.
...
%
However, none of these options improved simulation variance significantly and all internal setting were set restored to the default values. Details on this previous investigation can be found on the Trustworthy Systems GitHub~\cite{TSLUnrealEngineTesting}. \DIFaddbegin {\color{red} \DIFadd{NOT ACCESSIBLE}}
\DIFaddend 

\DIFaddbegin \subsection{\DIFadd{Initial Results and Discussion}}\label{InitialResultsSection}

{\color{red} \DIFadd{This is for unrestricted, so this does not come as a surprise later.}}
...
\textit{tolerance}, \DIFaddbegin {\color{red} \DIFadd{WHEN HAS THIS BEEN REPORTED? Should there be an ``Interim Results'' section?}}
\DIFaddend settings external to the game engine were explored. 
%
...
%
For high performance simulations the demand on these resources may be a significant fraction of the total available and an initial hypothesis was that as this ratio tended toward one, there would be an increase in \textit{simulation variance}. This hypothesis was supported by our initial work performed on the unreal engine~\cite{TSLUnrealEngineTesting} and was explored more fully in this work using the CARLA platform. \DIFaddbegin {\color{red} \DIFadd{From a reviewer's point of view this is a mysterious initial study that has been referred to often but nothing has been presented. Either take out or include, at least wrt.\ the overall observations, such as the support for this hypothesis.}}
\DIFaddend 
...
Resource utilisation was artificially increased for both CPU and GPU devices to include a range of values from \DIFdelbegin \DIFdel{0-95}\DIFdelend \DIFaddbegin \DIFadd{0 to 95}\DIFaddend \% (see Section\DIFaddbegin \DIFadd{~}\DIFaddend \ref{s:methodology}) using reported values of the system monitors \texttt{htop} and \texttt{nvidia-smi} respectively. Resource utilisation figures reported here should be considered approximate values.
...
%DIF > 
\DIFaddbegin {\color{red} \DIFadd{Somewhere the experimental setup needs to clarify that one set was executed ``unrestricted'' and another ``restricted'' - say what this means and why this was done.}}
\DIFaddend 


\todo[inline,color=green!40]{GC Addition (para below):} 
%DIF < Issues around floating point precision and concurrency as previously mentioned notwithstanding, 
%DIF > Issues around floating-point precision and concurrency as previously mentioned notwithstanding, 
Practitioners should also be aware that many libraries for calculating variance itself may require attention to get precise results. For example the \texttt{numpy} method of variance is sensitive to the input precision and will return an incorrect answer if the wrong parameters are set~\cite{NumpyVar}. In \texttt{matlab}, the calculation of variance may switch from single thread to a multi-threaded execution not obviously apparent to the user when the input data size becomes large enough, opening up the potential for \DIFdelbegin \DIFdel{concurrency }\DIFdelend \DIFaddbegin \DIFadd{concurrency-induced }\DIFaddend imprecision~\cite{matlab_parallel_computing}. 
...
% ***************************************************
\subsection{\DIFaddbegin \DIFadd{Experimental }\DIFaddend System Configuration and \DIFdelbegin \DIFdel{Screening}\DIFdelend \DIFaddbegin \DIFadd{Pre-Screening}\DIFaddend }\label{s:screening}
\DIFdelbegin \DIFdel{These tests }\DIFdelend \DIFaddbegin \DIFadd{The experiments }\DIFaddend were carried out on an Alienware Area 51 R5 with an i9 9960X processor with 64GB non-ECC DDR4 RAM at 2933MHz with an NVIDIA GeForce RTX 2080 GPU with 8GB GDDR6 RAM at 1515 MHz. 
%
\DIFdelbegin \DIFdel{Operating }\DIFdelend \DIFaddbegin \DIFadd{The operating }\DIFaddend systems was Linux Ubuntu 18.04.4 LTS. 
\DIFdelbegin \DIFdel{To ensure reliability of results, each test was repeated 1000 times. 
}\DIFdelend %
Tests were carried out in CARLA (v0.9.6 and Unreal Engine v4.22) using synchronous mode with a fixed $dt$ of $0.05$s. 
%
\DIFaddbegin \DIFadd{Each test was repeated 1000 times. }{\color{red} \DIFadd{Justify why 1000 - see my later comment in the Method section.}}
%DIF > 
\DIFaddend A detailed guide for reproducing \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend experiments along with the scripts used are provided on github\footnote{\url{https://github.com/TSL-UOB/CAV-Determinism/tree/master/CARLA_Tests_setup_guide}}.
%
To eliminate some of the potential sources of non-determinism outlined in Section~\ref{s:nondeterminisimSources} a series of screening tests and analyses were performed on our system. These were:
 ...
 \begin{itemize} [leftmargin=*]
    ...
    \item Graphical memory: \texttt{cuda\_memtest}~\cite{cuda_memtest}\DIFaddbegin \DIFadd{, }\DIFaddend \cite{shi2009testing}.
    \item Non-uniform memory access \DIFaddbegin \DIFadd{(NUMA)}\DIFaddend : \texttt{numactl}~\cite{numactl_NUMA} was used to fix the simulator and test script to single cores\DIFaddbegin \DIFadd{, }\DIFaddend but only a minor (2\%) improvement in simulation variance was observed\DIFdelbegin \DIFdel{and therefore }\DIFdelend \DIFaddbegin \DIFadd{. Therefore this was }\DIFaddend not used for subsequent testing. \DIFaddbegin {\color{red} \DIFadd{But if we aim to min the sources of non-det, then that should be considered/used?}}
...
% ***************************************************
\subsection{\DIFaddbegin \DIFadd{Final }\DIFaddend Results and Discussion}\DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{ResultsSection}%%%
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{FinalResultsSection}

{\color{red} \DIFadd{Consider spitting to introduce the first part of the table, which motivates the 2nd lot of experiments under load restrictions. I'd find this more logical.}}

\DIFaddend A summary of all the results \DIFdelbegin \DIFdel{are shown }\DIFdelend \DIFaddbegin \DIFadd{is shown in }\DIFaddend Table~\ref{TableOfExperiments}\DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{.  In }\DIFaddend the column $\max\sigma$ (unrestricted) \DIFdelbegin \DIFdel{, where }\DIFdelend the value reported is the maximum deviation across all resource utilisation levels, i.e.\DIFaddbegin \DIFadd{\ }\DIFaddend the worst case for a given scenario.  
%
From these results it is clear that scenarios with only pedestrian actors (\DIFdelbegin \DIFdel{tests 5-6}\DIFdelend \DIFaddbegin \DIFadd{Tests 5 \& 6}\DIFaddend ) display results within tolerance over all resource utilisation levels with or without a collision where $\max\sigma$ is $5.6\times10^{-13}$ or $0.56\si{\pico\metre}$. 
%DIF > 
However, all other scenarios involving vehicles or a mixture of actor types \DIFdelbegin \DIFdel{breach the tolerance that has been set}\DIFdelend \DIFaddbegin \DIFadd{do not meet the required tolerance}\DIFaddend , with some deviation in actor path as large as $59$cm.
%DIF > 
...
%
Clearly\DIFdelbegin \DIFdel{a deviation in results of }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend such a large \DIFdelbegin \DIFdel{amount is unacceptable if simulation is to be used as a reliable }\DIFdelend \DIFaddbegin \DIFadd{deviation cannot be acceptable for simulation to be considered a credible }\DIFaddend verification tool.

Resource utilisation \DIFdelbegin \DIFdel{level }\DIFdelend was found to have a significant impact on \textit{simulation variance}.
%
Figure~\ref{ExperimentsStressSummary} shows $\max\sigma$ against the artificially increased resource utilisation level, where the $x$-axis indicates the approximate percentage of resource utilisation (for CPU \& GPU). In this figure, \DIFdelbegin \DIFdel{anything }\DIFdelend \DIFaddbegin \DIFadd{any $\max\sigma$ }\DIFaddend above the $1$cm level (\DIFaddbegin \DIFadd{indicated by a }\DIFaddend dashed line) is considered non-permissible according to our \DIFdelbegin \DIFdel{specific tolerancelevel informed from our verification requirements}\DIFdelend \DIFaddbegin \DIFadd{specified tolerance}\DIFaddend . 
%
\DIFaddbegin \DIFadd{Note that the non-permissible results in Figure~\ref{ExperimentsStressSummary} (all those above the dashed line) are the worst case account of the situation, as per Equation~\ref{eq:max_sigma}, as the maximum variance is taken over the entire simulation period.
%DIF > 
}\DIFaddend \todo[inline]{Where are the verification requirements listed? They should be part of the scenario description. GC- added to the start of section 3.} 
%

A general pattern in the results indicates that \DIFdelbegin \DIFdel{; }\DIFdelend some scenarios consistently fail to produce results within \DIFdelbegin \DIFdel{permissible levels of variance at any resource utilisation level (}\DIFdelend \DIFaddbegin \DIFadd{tolerance, irrespective of resource utilisation (cf.\ }\DIFaddend Fig.~\ref{ExperimentsStressSummary} Test \DIFdelbegin \DIFdel{ID }\DIFdelend 2 \& 4 \DIFaddbegin \DIFadd{are }\DIFaddend above the dashed \DIFdelbegin \DIFdel{cm level), }\DIFdelend \DIFaddbegin \DIFadd{$1$cm line), while }\DIFaddend some are consistently within tolerance (\DIFdelbegin \DIFdel{Test ID }\DIFdelend \DIFaddbegin \DIFadd{cf.\ Fig.~\ref{ExperimentsStressSummary} Test }\DIFaddend 5 \& 6 \DIFaddbegin \DIFadd{both are }\DIFaddend with pedestrians only), and \DIFdelbegin \DIFdel{that }\DIFdelend some cases only fail to meet the tolerance requirement \DIFdelbegin \DIFdel{when at higher }\DIFdelend \DIFaddbegin \DIFadd{at higher resource }\DIFaddend utilisation levels, \DIFdelbegin \DIFdel{breaching the tolerance }\DIFdelend \DIFaddbegin \DIFadd{i.e.\ }\DIFaddend above 75\% resource utilisation (\DIFdelbegin \DIFdel{Test ID }\DIFdelend \DIFaddbegin \DIFadd{cf.\ Fig.~\ref{ExperimentsStressSummary} Test }\DIFaddend 1 \& 3). \DIFaddbegin {\color{red} \DIFadd{??? about 85\% based on the figure - or not???}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \todo[inline]{Help the reader see what you can see by explaining exactly where this can be seen in the figure(s). GC text added above}
%DIFDELCMD < 
...
\begin{figure}[t]
    ...
    \caption{\DIFdelbeginFL \DIFdelFL{Summary }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Variance range }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{investigating the effect of }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{three }\DIFaddendFL NICE priority \DIFdelbeginFL \DIFdelFL{setting }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{settings }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{0\% and 75\% additonal }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{additional }\DIFaddendFL CPU \& GPU resource utilisation \DIFaddbeginFL \DIFaddFL{of 0\% and 75\%}\DIFaddendFL .}
    \label{NICEExperimentStressSummary}
\end{figure}


The second key finding of this work is illustrated in Fig.~\ref{CarsPeopleCollsionsCG25}. In this scenario (\DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{Test }\DIFaddend 4), there is a collision between a vehicle (Car ID 2, solid line) and a pedestrian (Ped ID 3, dot dash line) which occurs at a simulation time of approximately $6$s and a second vehicle actor (Car ID 1, dashed line), which is \textit{not involved in the collision}. There are three observations\DIFdelbegin \DIFdel{here}\DIFdelend ; firstly that the vehicle directly involved in the collision (Car ID 2) displays high simulation variance immediately after the collision.
% as described above. 
Secondly, that the maximum deviation of the pedestrian involved in the collision (Ped ID 3) is at a tolerable level throughout the test\footnote{However, please note that in CARLA the pedestrian object is destroyed post-collision hence the flat line from $t=6$s onwards.}. Thirdly, we observed a delayed effect on Car ID 1 showing high simulation variance with a $5$s delay \textit{even though this vehicle was not involved in the collision}. This final point should be of particular concern to verification engineers\DIFdelbegin \DIFdel{and research practitioners }\DIFdelend \DIFaddbegin \DIFadd{, developers and researchers }\DIFaddend in the field as it implies that \textit{any collision between actors can affect the simulation variance of the entire actor population} and could potentially result in  erroneous \DIFdelbegin \DIFdel{assertion checking. }\DIFdelend \DIFaddbegin \DIFadd{simulation results. %DIF >  not just assertion checking 
}\DIFaddend 
...
\begin{figure*}[t]
    ...
    \caption{\DIFdelbeginFL \DIFdelFL{Flow diagram }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Stages }\DIFaddendFL of the \DIFdelbeginFL \DIFdelFL{methodology }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{method }\DIFaddendFL proposed to determine the \DIFdelbeginFL \DIFdelFL{simulation }\DIFdelendFL variance of a simulation. \DIFaddbeginFL {\color{red} \sc \DIFaddFL{please update to match text}}\DIFaddendFL }
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{To conclude, the }\DIFaddend main findings of this \DIFdelbegin \DIFdel{work }\DIFdelend \DIFaddbegin \DIFadd{case study }\DIFaddend suggest a working practice that would minimise the \DIFaddbegin \DIFadd{factors that give rise to the }\DIFaddend non-deterministic effects observed in this investigation. 
%DIF > 
By limiting simulation results to pre-collision data and ensuring resource utilisation levels do not exceed 75\%, the permissible variance of $1$cm is achievable as shown in the \textit{restricted} column in Table\DIFdelbegin \DIFdel{.}\DIFdelend ~\ref{TableOfExperiments}. By applying this set of restrictions upon the simulation the maximum observed deviation across all experiments was $0.98$cm which is within the target tolerance we set out to achieve. 
%
...
An investigation into the \DIFdelbegin \DIFdel{response }\DIFdelend \DIFaddbegin \DIFadd{impact }\DIFaddend of process scheduling on the simulation variance was undertaken \DIFaddbegin {\color{red} \DIFadd{say why}}\DIFaddend . The experiment was repeated ($n=1000$) using \DIFdelbegin \DIFdel{test }\DIFdelend \DIFaddbegin \DIFadd{Test }\DIFaddend 1 but altering the process scheduling priority using the program \texttt{NICE}.\footnote{\url{http://manpages.ubuntu.com/manpages/bionic/man1/nice.1.html}} Setting a higher priority for the simulator process with respect to the resource utilisation processes, it was possible to determine if scheduling could account for the increased simulation variance when the system is under high resource utilisation.
%
...
The results are presented in Fig.~\ref{NICEExperimentStressSummary} \DIFaddbegin {\color{red} \DIFadd{What is on the y axis?}} \DIFaddend where the box denotes the inter-quartile range \DIFaddbegin {\color{red} \DIFadd{of???}}\DIFaddend , non-outlier limits by the whiskers and a horizontal bar for the median. The figure shows that decreasing the priority \DIFaddbegin {\color{red} \DIFadd{of the simulator process}} \DIFaddend (right hand side of \DIFaddbegin \DIFadd{the  }\DIFaddend plot) has little effect on simulation variance when compared to a \DIFaddbegin \DIFadd{default }\DIFaddend NICE value of 0 (central \DIFdelbegin \DIFdel{plot bars }\DIFdelend \DIFaddbegin \DIFadd{bars in the plot}\DIFaddend ). Increasing priority (left hand side of \DIFdelbegin \DIFdel{plot) reduced }\DIFdelend \DIFaddbegin \DIFadd{the plot) significantly reduced the }\DIFaddend variance for the 75\% resource utilisation \DIFdelbegin \DIFdel{but this did }\DIFdelend \DIFaddbegin \DIFadd{experiment, but this does }\DIFaddend not account for all the difference in the observed results. This \DIFdelbegin \DIFdel{is demonstrated }\DIFdelend \DIFaddbegin \DIFadd{can be seen }\DIFaddend in the maximum priority setting where the bars in \DIFaddbegin \DIFadd{the }\DIFaddend plot are not equal, indicating an additional contribution to variance not accounted for by the NICE scheduling. 
%
\DIFdelbegin \DIFdel{This unaccounted }\DIFdelend \DIFaddbegin \DIFadd{The remaining }\DIFaddend difference in the variance \DIFaddbegin \DIFadd{between the two resource utilisation levels }\DIFaddend may be due to the lack of absolute control that \texttt{NICE} has over \DIFdelbegin \DIFdel{the }\DIFdelend process scheduling.\footnote{\url{https://askubuntu.com/questions/656771/process-niceness-vs-priority}}

\subsection{Investigation Summary} \label{s:empirical_summary} \DIFaddbegin {\color{red} \DIFadd{Factor into overall conclusion of the case study}}
\DIFaddend 

% \todo[inline]{GC-need to reference the 'restricted' set of results here too}

\DIFdelbegin %DIFDELCMD < \todo[inline]{Do we need to say here why we think certain results are non-det, what the reasons might be?? The results indicate that actor type, resource utilisation and collision events all affect the determinism in the CARLA simulator. Collision events may utilise more computational resources than simulations without such events, i.e. the physics loop takes longer to process as there are complex physical interactions to resolve. This may coincide with the second observation that higher resource levels also lead to non-deterministic results. We hypothesise that both these observations are due to improper management of thread scheduling within the engine. When resources are occupied there may be a delay in processing critical calculations for the simulator, which may arrive out of an expected sequence or the engine compensates for this delay by reducing the number of physics loops that can be performed within a render cycle. (is this 2nd one true, or does carla ignore render requests?)}
%DIFDELCMD < %%%
\DIFdelend %DIF > \todo[inline]{Do we need to say here why we think certain results are non-det, what the reasons might be?? The results indicate that actor type, resource utilisation and collision events all affect the determinism in the CARLA simulator. Collision events may utilise more computational resources than simulations without such events, i.e. the physics loop takes longer to process as there are complex physical interactions to resolve. This may coincide with the second observation that higher resource levels also lead to non-deterministic results. We hypothesise that both these observations are due to improper management of thread scheduling within the engine. When resources are occupied there may be a delay in processing critical calculations for the simulator, which may arrive out of an expected sequence or the engine compensates for this delay by reducing the number of physics loops that can be performed within a render cycle. (is this 2nd one true, or does carla ignore render requests?)}

This empirical investigation has highlighted the shortcomings of using a games engine for simulation based verification and advice for best working practice. However, these results are specific to the hardware and software used in the study and may not be transferable to other systems directly. 
\DIFdelbegin \DIFdel{Therefore we now present }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{Therefore we have derived }\DIFaddend a general methodology that practitioners can follow to find the 
% \textit{deterministic domains of operation} 
\textit{operational domains of permissible variance} 
for \DIFdelbegin \DIFdel{any hardware configuration. }\DIFdelend %DIF > any hardware configuration.
\DIFaddbegin \DIFadd{a game-engine-based simulation environment. This methodology is presented in the next section. 
}\DIFaddend 
...
%  METHOD
% ***************************************************
\section{\DIFdelbegin \DIFdel{Methodology}\DIFdelend \DIFaddbegin \DIFadd{Method to determine the Variance of a Simulation}{\color{red} \DIFadd{for what???}}\DIFaddend } \label{s:methodology}
In this section a method for \DIFaddbegin \DIFadd{assessing the actor path variance in AV simulation environments that are based on game engines is presented in the form of a work flow, see Fig.~\ref{method_diagram}. This method can be used to determine the operational domains of permissible variance of a simulation environment.  
%DIF > 
}{\color{red} \DIFadd{MAKES NO SENSE: }{\sc \DIFadd{In this section a method for }\DIFaddend determining the variance of simulated actor path trajectories \DIFaddbegin \DIFadd{(KIE: a trajectory is a path), termed simulation variance}\DIFaddend , \DIFdelbegin \DIFdel{termed }\textit{\DIFdel{simulation variance}}%DIFAUXCMD
\DIFdel{, }\DIFdelend and resolving the operational domains of permissible variance of the simulation presented here as a work flow, see Fig.~\ref{method_diagram}.\DIFaddbegin }}
\DIFaddend %
In addition\DIFdelbegin \DIFdel{a number of }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend recommendations and best \DIFdelbegin \DIFdel{practices are suggested that other practitioners can follow to minimise simulation variance . 
}\DIFdelend \DIFaddbegin \DIFadd{practice guidelines for minimising simulation variance are suggested. }{\color{red} \sc \DIFadd{Highlight these in the text.}}
\DIFaddend % A number of parameters have been identified either theoretically or from the empirical investigation that influence simulation variance. 
...
\DIFadd{The method consists of a sequence of five stages; experimental design, simulator configuration, external configuration, execution and analysis. In the following, each stage is described in detail with reference to the items listed for each stage in Fig.~\ref{method_diagram}. }{\color{red} \DIFadd{NOTE: renamed stages.}}

\DIFaddend \subsection{\DIFaddbegin \DIFadd{Experimental }\DIFaddend Design\DIFdelbegin \DIFdel{Experiment}\DIFdelend }\label{s:design_experiment}

\DIFaddbegin {\color{red} \sc \DIFadd{Surely, a series of systematically designed experiments, each repeated X times, should be set up, where each experiment varies one of the suspected sources of non-determinism? Then, the analysis of the data from each experiment should reveal the actual sources of non-determinism for the given setting? In other words, we are aiming to fix as much as we can control (easily), and then we intentionally vary (ideally) single variables, e.g.\ the resource utilisation, to find out what makes a difference. We need a clear definition of the objectives of the experimental design, i.e.\ what one should vary and what one wants to ``nail down''.}}

\DIFaddend \subsubsection{Actors} \label{s:actors}

Different actor types may be handled differently by the game engine\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{. For instance, }\DIFaddend our investigation indicated that CARLA pedestrians (termed \textit{walkers}) do not suffer from simulation variance under any conditions that were tested. However\DIFaddbegin \DIFadd{, }\DIFaddend the introduction of CARLA vehicles increased the simulation variance of the pedestrian-only test by 12 orders of magnitude. \DIFaddbegin {\color{red} \sc \DIFadd{This needs rephrasing as it appears to contradict the statement above.}} \DIFaddend As such, all actors that could be included in any future simulation should be tested, including any non-standard CARLA or bespoke actors. \DIFaddbegin {\color{red} \sc \DIFadd{... such as the ego vehicle...? }[\DIFadd{see section on ``External Settings'' last point - Ego Controller.}]}
\DIFaddend %
...
Actor paths, or the sequence of actions required to generate paths, should be hard-coded to ensure repeatability. These paths should include collisions between actors and potentially collisions between actors and static scenery if this is likely to occur in the simulation or as part of the verification process. Actor paths without collision are also important to include as these will serve as a baseline to the other tests. \DIFaddbegin {\color{red} \sc \DIFadd{What baseline?}}
\DIFaddend 

\subsubsection{\DIFdelbegin \DIFdel{Setting a Tolerance}\DIFdelend \DIFaddbegin \DIFadd{Permissible Variance}\DIFaddend } \label{s:threshold}
The verification engineer should set the \DIFaddbegin \textit{\DIFadd{permissible variance}}\DIFadd{, also termed }\DIFaddend \textit{tolerance}\DIFdelbegin \DIFdel{based on their specific requirements which refers to the acceptable level of actor path variance.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  which refers to the acceptable level of actor path variance. 
}\DIFaddend % ($\sigma^2$), which is quoted in terms of deviation ($\sigma$) for ease of interpretation. 
\DIFdelbegin \DIFdel{This tolerance level is analogous to the analogue signal level associated with each of the binary states %DIF < of the signals 
...
The tolerance \DIFdelbegin \DIFdel{that the user must set }\DIFdelend depends on the objectives of the simulation and the granularity at which the simulation environment operates. For example, this tolerance must be sufficiently small to enable accurate assertion checking \DIFdelbegin \DIFdel{. In our empirical investigation we set }\DIFdelend \DIFaddbegin \DIFadd{and coverage collection. In Section~\ref{s:tolerance} }\DIFaddend a tolerance of $1$cm \DIFdelbegin \DIFdel{which is }\DIFdelend \DIFaddbegin \DIFadd{was considered }\DIFaddend sufficient for urban scenario assertion checking. In practice, it may be necessary to determine this tolerance experimentally. 

%DIF > The permissible variance is analogous to the analogue signal level associated with each of the binary states %of the signals in digital circuits. At some low analogue voltage the signal is interpreted as binary $0$, this may be, for example, any voltage below $0.5$V, and the same signal is interpreted as binary $1$ at some higher voltage, e.g.\ above $3.5$V. This convention enables the abstraction from the noisy physical signal values that are analogue voltages into the digital domain with binary representation. <- KIE: meant only to illustrate that other areas also have defined tolerances. 
%DIF > 
\DIFaddbegin 


\DIFaddend % \todo[inline]{Make sure assertion checking has previously been introduced.}
...
\subsubsection{Simulation \DIFdelbegin \DIFdel{Length}\DIFdelend \DIFaddbegin \DIFadd{Time}\DIFaddend } \DIFaddbegin {\color{red} \sc \DIFadd{Should this not be part of the Execution section?}}
\DIFaddend The simulation time should be sufficient to record \DIFdelbegin \DIFdel{an interaction }\DIFdelend \DIFaddbegin \DIFadd{interactions }\DIFaddend between actors but not so long that the testing take an inconvenient time to complete. In our empirical investigation a simulation time of $10-20$s was sufficient to monitor the distinct change in events such as the pre- and post-collision including the delayed effect seen by other actors, shown in Fig.~\ref{CarsPeopleCollsionsCG25}. \DIFaddbegin {\color{red} \sc \DIFadd{Point to the section that specified that.}}
\DIFaddend 
...
The settings internal to the game engine or other simulation environment should be set to ensure a fixed physics time step, $dt$. If using CARLA\DIFaddbegin \DIFadd{, }\DIFaddend a small fixed value, say $0.05s$, can be set by using \DIFdelbegin \DIFdel{the following: }\DIFdelend \texttt{setting.fixed\_delta\_seconds = 0.05}\DIFdelbegin \DIFdel{, whereas in Unity}\DIFdelend \DIFaddbegin \DIFadd{. In Unity, }\DIFaddend the default fixed time step is set to $0.02s$~\cite{MonoBehaviour_unity}. 
\DIFaddbegin 

\DIFaddend In CARLA, synchronous mode must be used to allow communication to external controllers which ensures no sensor data are passed out of order to the simulator which is particularly important if a complex ego controller is used~\cite{carla_sim_config}. 
%DIF < 
\DIFaddbegin 

\DIFaddend The use of random numbers must be controlled through \DIFdelbegin \DIFdel{the use of fixed seedswhich }\DIFdelend \DIFaddbegin \DIFadd{fixed seeds, resulting in pseudo-randomness. Random numbers }\DIFaddend might be used \DIFaddbegin \DIFadd{in the simulation environment }\DIFaddend to control variations of background effects, e.g.\DIFaddbegin \DIFadd{\ }\DIFaddend weather patterns, or the navigation of random pedestrian actors, external vehicle controllers or other clients connected to the simulation environment. 
%
...
%DIFDELCMD < %%%
\DIFdelend %DIF > 
The version number of the CARLA and Unreal environment has also be shown to affect results, see~\cite{TSLUnrealEngineTesting}\DIFdelbegin \DIFdel{, so ensuring }\DIFdelend \DIFaddbegin \DIFadd{. Therefore, ensuring a }\DIFaddend consistent version number throughout testing is also important.
...
The resources available to the simulator have been shown to have a significant effect on the \DIFdelbegin \DIFdel{simulation variance of simulated vehicles and exploring this as a variable the analysis is required.
}\DIFdelend \DIFaddbegin \DIFadd{variance of the path of simulated vehicles. %DIF > and exploring this as a variable the analysis is required. 
%DIF > 
Thus, it is important to understand at what level of resource utilisation the system running the simulation becomes susceptible to }\textit{\DIFadd{simulation variance}}\DIFadd{.
}

\DIFaddend CPU utilisation software, such as the linux \texttt{stress} tool\DIFaddbegin \DIFadd{, }\DIFaddend which is a workload generator program\DIFaddbegin \DIFadd{, }\DIFaddend can be used \DIFaddbegin \DIFadd{to }\DIFaddend spawn workers on any number of cores or virtual threads on a system. This can be used to artificially \DIFdelbegin \DIFdel{increased }\DIFdelend \DIFaddbegin \DIFadd{increase }\DIFaddend the load on the system\DIFdelbegin \DIFdel{to explore at what point your system may become susceptible to }\textit{\DIFdel{simulation variance}}%DIFAUXCMD
\DIFdelend . For GPU utilisation\DIFaddbegin \DIFadd{, }\DIFaddend \texttt{gpu-burn} \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend employed using the \texttt{fur test}\DIFdelbegin \DIFdel{where different }\DIFdelend \DIFaddbegin \DIFadd{. Different }\DIFaddend resolutions and multiple instances can be used to tune graphical utilisation levels~\cite{GPU_stress}. Reported values of resource utilisation can be obtained using the system monitors \texttt{htop} and \texttt{nvidia-smi} for CPU and GPU\DIFaddbegin \DIFadd{, }\DIFaddend respectively. These values \DIFdelbegin \DIFdel{can also be written into the data logging for completeness.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{should be added to the data logs.
%DIF > 
}\DIFaddend Alternatively, in place of artificial resource utilisation\DIFaddbegin \DIFadd{, }\DIFaddend multiple instances of the simulation could be executed simultaneously. However, the granularity of control with this approach may be reduced.
...
Prior to experimental execution the system hardware should be tested for memory conformity and to ensure no single bit errors are occurring, see \DIFaddbegin \DIFadd{Section}\DIFaddend ~\ref{s:screening}. For mainboard memory \texttt{memtest86}\ can be used on most platforms to run a series of pre-defined memory test patterns. This memory testing software can also be used for ECC enabled hardware. Similarly, to test memory on Nvidia based graphical adaptors \texttt{cuda\_memtest} can be used to ensure no memory errors exist.


\subsubsection{Scheduling}
We hypothesise that thread scheduling may be a major contributor to the non-deterministic results found in the empirical study. However, \DIFaddbegin \DIFadd{gaining }\DIFaddend fine control over the scheduling policy and thread execution order \DIFdelbegin \DIFdel{may be non-trivial. 
Such policies are defined by the operating system and may execute threads with equal priority. In }\DIFdelend \DIFaddbegin \DIFadd{is known to not be trivial~\mbox{%DIFAUXCMD
\cite{acm-q-rr-interview}}%DIFAUXCMD
. 
}

\DIFadd{The operating system schedules threads according to a specified scheduling policy, potentially based on equal thread priority. Thus, in }\DIFaddend such a case, all tasks non-essential to the simulator should be terminated to prevent \DIFdelbegin \DIFdel{interruption to the simulator}\DIFdelend \DIFaddbegin \DIFadd{interference with the simulation}\DIFaddend . 

% If the policy or operating system cannot be changed then s
\DIFdelbegin \DIFdel{Setting the simulator process to }\DIFdelend \DIFaddbegin \DIFadd{Assigning }\DIFaddend a higher priority \DIFaddbegin \DIFadd{to the simulator process }\DIFaddend may help to alleviate conflicting task scheduling which can be achieved by using, for example \texttt{TaskSettings.Priority} in Windows~\cite{TaskSettingWindows} or \texttt{NICE} in Linux~\cite{Nice_linux}.

\subsubsection{NUMA Control}
Control over \DIFaddbegin \DIFadd{a }\DIFaddend Non-Uniform Memory Access policy can be achieved using \texttt{numactl} for multiprocessors with shared memory. This control allows the simulator program to be fixed on a single core\DIFdelbegin \DIFdel{reducing }\DIFdelend \DIFaddbegin \DIFadd{, reducing and unifying }\DIFaddend memory access time.
%
\DIFdelbegin \DIFdel{Initial screening tests done }\DIFdelend \DIFaddbegin \DIFadd{Note that initial screening tests performed }\DIFaddend with NUMA control \DIFdelbegin \DIFdel{gave moderate }\DIFdelend \DIFaddbegin \DIFadd{resulted in only minor }\DIFaddend improvements in simulation variance\DIFdelbegin \DIFdel{of the order of a few percent}\DIFdelend , see Section~\ref{s:screening}. 
%DIF > 
This control may assist if simulation variance is borderline to the tolerance but not seen as essential. \DIFaddbegin {\color{red} \sc \DIFadd{Unclear: what is not essential?}}
\DIFaddend 

\subsubsection{Ego Vehicle Controller \DIFaddbegin {\color{red} \sc \DIFadd{Why is this not covered under actors? }[\DIFadd{see my note in the Actors section}]}\DIFaddend }
\DIFaddbegin {\color{red} \sc \DIFadd{Clearly separate the investigation from the Method. The fact that the case study did not have an ego vehicle should be mentioned as a limitation of the case study, rather than elaborated on here, but why does this make a difference?}}
\DIFadd{The impact of the ego vehicle  should be considered as an actor but ensuring that any randomness used in the controller is controlled with fixed seeds and that any adaptive learning algorithms should be controlled in the same manner. }{\color{red} \sc \DIFadd{Long and cumbersome sentence with strange grammar: }\DIFaddend No ego vehicle was used in the empirical investigation but this should be considered as an actor but ensuring that any randomness used in the controller is controlled with fixed seeds and that any adaptive learning algorithms should be controlled in the same manner. \DIFaddbegin }
\DIFaddend 
...
%DIF > 
Initial testing~\cite{TSLUnrealEngineTesting} indicated an actor path deviation of $1\times10^{-13}$cm for 997 out of 1000 tests\DIFdelbegin \DIFdel{but with 3 }\DIFdelend \DIFaddbegin \DIFadd{, with three }\DIFaddend tests reporting a deviation of over {\raise.17ex\hbox{$\scriptstyle\sim$}}$10$cm. While executing 100 repeats may seem sufficient, this sample size may fail to observe \DIFdelbegin \DIFdel{these }\DIFdelend events that occur with low probability, \DIFdelbegin \DIFdel{inferring a }\DIFdelend \DIFaddbegin \DIFadd{giving }\DIFaddend false confidence in the results. \DIFaddbegin {\color{red} \sc \DIFadd{Again, separate the study from the method. This observation should really have been made in the section on ``Experimental System Config and Pre-Screening, to justify the figure of 1000 repeats.}} \DIFaddend It is therefore recommended that the sample size is \DIFdelbegin \DIFdel{found }\DIFdelend \DIFaddbegin \DIFadd{determined }\DIFaddend empirically dependent on the number of results that exceed the tolerance. \DIFaddbegin {\color{red} \sc \DIFadd{How would one know?}}
\DIFaddend % To ensure outlier data points are not hidden in the average of a large number of tests, the maximum deviation should be reported rather than the average when summarising.

\subsubsection{Data Logging}
\DIFdelbegin \DIFdel{Data logging should be used to record the actorpositions }\DIFdelend \DIFaddbegin \DIFadd{Unique identifiers should be assigned to each experiment, each repeat and each individual actor. 
The time-stamped actor positions should then be recorded }\DIFaddend at fixed time intervals throughout the simulation in order to determine the variance in actor path. Additional information \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{should }\DIFaddend also be logged such as the CPU and GPU utilisation levels and engine specific metrics such as game loop latency.
% such as the \texttt{unit} command in Unreal Engine. 
\DIFdelbegin \DIFdel{For the subsequent analysis, the actor should have an identifier and the repeat number and simulation time should also be captured. 
}\DIFdelend %DIF > For the subsequent analysis, each actor should have an identifier. The repeat number and simulation time should also be captured. 
...
\subsection{Analysis}
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{For each experiment, the }\DIFaddend maximum value of actor path deviation over all time samples and actors, $\max\sigma$, should be analysed to identify which of the candidate sources of non-determinism require restriction or control \DIFdelbegin \DIFdel{for permissible operation. 
Finding these boundaries will identify the }\textit{\DIFdel{domains of permissible variance}} %DIFAUXCMD
\DIFdel{for the user specific hardware.}\DIFdelend \DIFaddbegin \DIFadd{to reach the domain of permissible variance within the simulation environment. 
%DIF > Finding these boundaries will identify the \textit{domains of permissible variance} for the user specific hardware.
%DIF >  KIE: Why now point to HW? And, which "boundaries"?
}\DIFaddend 

\DIFaddbegin {\color{red} \sc \DIFadd{Unclear how this relates to Analysis. Is this a summary of the method? If so, it should be moved to the front or to the conclusion.}} \DIFaddend This method can be extended to a broader actor states and actions including actor orientation, speed, and any other status indicators that may be of interest and also including appropriate actions that may be useful for verification purposes. This broader As such, Equation~\ref{eq:max_sigma} would need to be adjusted to include these new variables. \DIFaddbegin {\color{red} \sc \DIFadd{Something is missing in this sentence - please correct before moving to a different place, or removing.}}
\DIFaddend 
...
\DIFdelbegin \DIFdel{The autonomous vehicle community are adopting game engine simulators for such purposes as control system }\DIFdelend %DIF > The autonomous vehicle community are adopting game engine simulators for such purposes as control system development and verification. 
\DIFaddbegin \DIFadd{Game engines offer simulation environments that are used for the }\DIFaddend development and verification \DIFaddbegin \DIFadd{of autonomous driving functions}\DIFaddend . 
%
\DIFdelbegin %DIFDELCMD < \todo[inline,color=red!40]{old:Having a deterministic simulator is required for reliable and repeatable results,}
%DIFDELCMD < %%%
\DIFdel{Having a deterministic }\DIFdelend \DIFaddbegin \DIFadd{Determinism of a }\DIFaddend simulator is required \DIFdelbegin \DIFdel{for precise results, }%DIFDELCMD < \todo[inline]{Having a deterministic simulator is required for precise results,} 
%DIFDELCMD < %%%
%DIF < 
\DIFdelend \DIFaddbegin \DIFadd{to achieve repeatability, which is essential }\DIFaddend to find and fix software bugs \DIFdelbegin \DIFdel{and ensure }\DIFdelend \DIFaddbegin \DIFadd{efficiently, and to ensure simulation }\DIFaddend results are trustworthy. 
%
If a simulator is non-deterministic then practitioners should at least be aware of, and \DIFaddbegin \DIFadd{know }\DIFaddend how to find, the operational domains \DIFdelbegin \DIFdel{for reliable results. 
During an }\DIFdelend \DIFaddbegin \DIFadd{where }\textit{\DIFadd{simulation variance}} \DIFadd{is tolerable. 
}

\DIFadd{An }\DIFaddend investigation into the CARLA simulator \DIFdelbegin \DIFdel{, a }\textit{\DIFdel{simulation variance}} %DIFAUXCMD
\DIFdel{was observed }\DIFdelend \DIFaddbegin \DIFadd{revealed a significant simulation variance }\DIFaddend for repeated tests with the same initial conditions \DIFdelbegin \DIFdel{indicating non-deterministic execution}\DIFdelend \DIFaddbegin \DIFadd{and event history, indicating non-determinism of the simulation}\DIFaddend . 
%
\DIFdelbegin \DIFdel{During the study we hypothesized and uncovered several parameters }\DIFdelend \DIFaddbegin \DIFadd{We then researched, identified and discussed potential sources for non-determinism in this context.
%DIF > 
A systematic case study of the CARLA simulator uncovered the actual factors }\DIFaddend that contribute towards greater simulation variance\DIFdelbegin \DIFdel{and hence }\DIFdelend \DIFaddbegin \DIFadd{, giving rise to }\DIFaddend non-deterministic execution\DIFdelbegin \DIFdel{, such as }\DIFdelend \DIFaddbegin \DIFadd{. In particular, }\DIFaddend actor collisions and \DIFdelbegin \DIFdel{system resource utilisation }\DIFdelend \DIFaddbegin \DIFadd{system-level resource utilisation were identified as key contributors}\DIFaddend . 
%
\DIFdelbegin \DIFdel{The results of the investigation are hardware and software version specific, so we proposed a general methodology that }\DIFdelend \DIFaddbegin {\color{red} \sc \DIFadd{Say what the recommendations were for the required tolerance of the case study, i.e.\ summarise our results/recommendations here and point to github for data/detail. This seems to have been commented out but was here before.}}

\DIFadd{A general method to assess the actor path variance of a game-engine-based simulation environment is then proposed. The method }\DIFaddend can be used to find the \textit{domains of permissible variance} \DIFdelbegin \DIFdel{for any }\DIFdelend \DIFaddbegin \DIFadd{of a simulation environment for a }\DIFaddend given system configuration. %DIF < 
\DIFdelbegin \DIFdel{This methodology will allow the AV verification community using simulation tools to ensure their results are reliable and trustworthy. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{This can give AV developers and verification engineers increased confidence in the simulation results and reduce debug time. 
%DIF >  
}\DIFaddend % \subsection{Future Work}
\DIFdelbegin \DIFdel{A potential avenue of exploration is developing a version of the simulator which has been designed specifically for verification. In this version the simulator would be deterministic because the execution is suitably controlled along with sources of }\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{As future work the method can be extended to criteria other than the actor path, e.g.\ actor orientation and any status indicators that may be of interest, also including actions, sequences and timings that may be useful for verification purposes.
}

\DIFadd{An ambitious avenue for future work is the development of a deterministic simulator for AV development and verification. This requires controlling all potential sources of non-determinism, including }\DIFaddend randomness and scheduling\DIFdelbegin \DIFdel{. A similar task was achieved with the }\DIFdelend \DIFaddbegin \DIFadd{, very similar to the development of the record-and-reply debugger }\DIFaddend \texttt{\DIFdelbegin \DIFdel{RR}%DIFDELCMD < } %%%
\DIFdel{program~\mbox{%DIFAUXCMD
\cite{RR_link}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{rr}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{RR_link}}%DIFAUXCMD
, originally developed to catch low-frequency non-deterministically failing tests at Mozilla~\mbox{%DIFAUXCMD
\cite{acm-q-rr-interview}}%DIFAUXCMD
}\DIFaddend .

\DIFdelbegin \DIFdel{With the advent of more AI systems becoming adaptive, meaning that the system may change it's response to the same stimulus over time, the notion of repeatability will need reassessing in the context of verification. The main research question here is what will constitute `the same' which will be important for passing or failing verification tests but also for determining coverage. 
}\DIFdelend %DIF > With the advent of more AI systems becoming adaptive, meaning that the system may change it's response to the same stimulus over time, the notion of repeatability will need reassessing in the context of verification. The main research question here is what will constitute `the same' which will be important for passing or failing verification tests but also for determining coverage. 

%DIF <  \todo[inline]{Review/rewrite this to match the earlier changes in the paper. Practitioners in this field can resolve the non-deterministic operational domains of their simulation by following the method outlined in this paper. Potential sources of non-deterministic simulation behaviour have been discussed such as floating point rounding error and scheduling policy. A case study was used to ascertain the boundary between deterministic and non-deterministic operational domains of the CARLA driving simulator. Results suggest non-deterministic outcomes when resource utilisation levels exceeded 75\% or when vehicle collisions occur for the particular hardware used in this study. Furthermore it was found that all actors, even those not involved directly in a collision, will behave non-deterministically if a collision callback to the game engine is made. Best operating practice follows that the simulation should be stopped as soon as a collision callback is detected or resource utilisation levels exceed 75\% although this value may be system specific. }
%DIF >  \todo[inline]{Review/rewrite this to match the earlier changes in the paper. Practitioners in this field can resolve the non-deterministic operational domains of their simulation by following the method outlined in this paper. Potential sources of non-deterministic simulation behaviour have been discussed such as floating-point rounding error and scheduling policy. A case study was used to ascertain the boundary between deterministic and non-deterministic operational domains of the CARLA driving simulator. Results suggest non-deterministic outcomes when resource utilisation levels exceeded 75\% or when vehicle collisions occur for the particular hardware used in this study. Furthermore it was found that all actors, even those not involved directly in a collision, will behave non-deterministically if a collision callback to the game engine is made. Best operating practice follows that the simulation should be stopped as soon as a collision callback is detected or resource utilisation levels exceed 75\% although this value may be system specific. }
