...
%DIF ADD cav_determinisim_main.tex            Thu Mar  4 13:55:02 2021
...
%DIF > {\color{red} PROBLEMATIC: Even operations such as the average of an array can result in issues with overflows and underflows {\sc (these can happen with Integers too)}
%DIF > and,  non-associative addition~\cite{Kapre2007} {\sc (only when the exec order is changed)} which may result in non-deterministic behaviour. }
...
\DIFaddend %
%DIF <  SEE BELOW \todo[inline]{Check the points on floating point operations in An Empirical Analysis of Flaky Tests~\cite{empirical-analysis-of-flaky-tests}.}
\DIFdelbegin \DIFdel{Even operations such as the average of an array can result in issues with overflows, underflows and non-associative addition~\mbox{%DIFAUXCMD
\cite{Kapre2007}}%DIFAUXCMD
which may result in non-deterministic behaviour. 
Calculation results may differ }\DIFdelend \DIFaddbegin \DIFadd{As a consequence, floating-point arithmetic is not associative. Thus, for arithmetic expressions beyond two operands, such as $x+y+z$, executing $x+(y+z)$ rather than $(x+y)+z$, can produce different results~\mbox{%DIFAUXCMD
\cite{Kapre2007}}%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{per se. 
%DIF > {\color{red} Should this be IMPRECISION rather than incorrectness?}
}\DIFaddend %
...
Previous investigations into the Unreal Engine indicated that collisions between \DIFdelbegin \DIFdel{dynamic }\DIFdelend actors and solid objects, termed \textit{blocking physics bodies} in Unreal Engine documentation~\cite{collision_overview}, can lead to high simulation variance\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{~}\DIFaddend \cite{TSLUnrealEngineTesting}. 
%
...
%
\DIFdelbegin \DIFdel{Repeated traces are compared at the same time index $t$ for actor $a$ to provide }\DIFdelend \DIFaddbegin \DIFadd{The logs from repeated tests are sourced to establish }\DIFaddend a value for the variance \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{associated with each actor, $a$, at each time point $t$, giving a variance function over time for each actor, }\DIFaddend $\sigma_a^{2}(t)$.
%DIF < 
\DIFdelbegin \DIFdel{Herein }\DIFdelend \DIFaddbegin 

\DIFadd{Instead of using ..., }{\color{red} \sc \DIFadd{please complete}} \DIFadd{herein }\DIFaddend the results are given in terms of the deviation, $\sigma_a(t)$, which indicates the dispersion of the actor position data relative to the mean and is helpfully in the same units as actor position, \DIFaddbegin \DIFadd{i.e.\ metres (}\DIFaddend $m$\DIFaddbegin \DIFadd{)}\DIFaddend , for ease of interpretation.
...
\DIFdelbegin \DIFdel{and therefore the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend maximum deviation is \DIFdelbegin \DIFdel{simply }\DIFdelend \DIFaddbegin \DIFadd{the absolute value of the }\DIFaddend square root of the maximum variance and herein referred to as ${\max\sigma}$ for brevity. 
%DIF >  {\color{red} So, ``a'' is any actor or a specific actor? Likewise, ``t'' is any time point or a specific time point? NOTE that any actor any time means a in A and t in T, where A is the set of actors and T is the set of all time points.}
...
%  METHOD
% ***************************************************
\section{\DIFdelbegin \DIFdel{Methodology}\DIFdelend \DIFaddbegin \DIFadd{Method to determine the Variance of a Simulation }{\color{red} \DIFadd{Say for what???}}\DIFaddend } \label{s:methodology}
...
\subsection{Analysis}
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{For each experiment, the }\DIFaddend maximum value of actor path deviation over all time samples and actors, $\max\sigma$, should be analysed to identify which of the candidate sources of non-determinism require restriction or control \DIFdelbegin \DIFdel{for permissible operation. Finding these boundaries will identify the }\textit{\DIFdel{domains of permissible variance}} %DIFAUXCMD
\DIFdel{for the user specific hardware. 
}\DIFdelend \DIFaddbegin \DIFadd{to reach the domain of permissible variance within the simulation environment. 
...
\DIFdelbegin \DIFdel{This method can be extended to a broader actor states and actions including actor orientation, speed, and any other status indicators that may be of interest and also including appropriate actions that may be useful for verification purposes. This broader As such, Equation~\ref{eq:max_sigma} would need to be adjusted to include these new variables.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % Game engine actors can be used to simulate the AV and other vehicles as well as pedestrians. For the experiment, actors should follow a series of trajectory waypoints which are hard-coded for each test type. 
