...
\author[2,3]{S\'everin Lemaignan\thanks{$^{2}${\footnotesize \{severin.lemaignan, tony.pipe\}@brl.ac.uk}}}
...
%
A case study using CARLA, an open-source autonomous driving simulation environment powered by Unreal Engine (game engine), illustrates the proposed method and results from an example scenario demonstrate simulation precision to an acceptable tolerance.
%
\textbf{ALTERNATIVE: This empirical investigation using CARLA, an open-source autonomous driving simulation environment powered by Unreal Engine, has highlighted the shortcomings of using a games engine for simulation based verification and advice for best working practice.}
...
%
CARLA\footnote{\url{http://carla.org/}}, for instance, is an open-source simulator for autonomous driving that is implemented in the Unreal Engine\footnote{\url{https://www.unrealengine.com/}}, a real-time 3D creation environment for the gaming and film industry as well as other creative sectors~\cite{CARLA_paper}. 
...
\subsection{Definition: Determinism}
...
%
However, if a simulation is non-deterministic then then there will be a variation in the generated output data. 
...
We adopt some terms from the mechanical engineering and statistics domain that best describe this situation~\cite{ADictionaryofMechanicalEngineering}.
%
...
%
\textit{Tolerance} is defined as the permissible limit of the variance. 
...
As an analogy, the simulator can be thought of as a manufacturing process, producing data and to find the value of the precision then the output must be measured for differences. Those differences describe the spread or variance in the process output and a hard limit on the variance can be defined, beyond which the output fails to meet the required tolerance, e.g. rejected for quality control.
%
...
If a simulator is deterministic then it will produce results with absolute precision or zero variance and hence will be within acceptable tolerance. If the simulator is non-deterministic then there will be a variance in the output which can be measured.
%

\subsection{Definition: Accuracy}

Precision and tolerance are not to be confused with \textit{accuracy} which describes how closely the generated output of a process aligns to a known standard. We therefore define accuracy as the difference between the true value (or reference value) and what has been achieved in the simulation or generation process. 
%
For a driving simulation the reference value, for which accuracy can be measured against, may be the real world that it seeks to emulate, where any divergence from this standard is termed the \textit{reality gap}. In most cases such accuracy will not be possible due to computational demands of such an exact replica and in most cases is unnecessary, where some authors state that `just the right amount of realism' is required to achieve valid simulation results~\cite{Koopman2018}. 


\subsection{Definition: Variance Measurement}

If the simulator is non-deterministic then how can this variance be measured?
%
This can be achieved from the actor state, or more specifically from the statistical variance of actor paths within a given scenario with a fixed set of actions. Actor path variance is chosen over other measures as this distance based metric forms the basis for many downstream verification tests such as assertion checking. 

The term \textit{simulation variance} refers to a measure of actor state variance in the simulation assuming fixed actions. Results are referred to in terms of deviation, rather than variance, as this is a more intuitive dimension to comprehend when considering verification tests, e.g. vehicle close-passing. Moreover, the maximum observed value of deviation was chosen over a mean value to ensure sufficient sensitivity.

\subsection{Definition: Simulation Trace}

A simulation trace is the output log from the simulator which is a time series of all actor positions ($x,y,z$) at regular intervals based on the time-step $dt$. This definition could be extended to include other variables, but position is sufficient for this analysis.


\subsection{Definition: Scene, Scenario \& Situation}
...
Determinism is a key prerequisite for simulation during AV development and testing. A deterministic simulation environment guarantees that tests are repeatable. Using mechanical engineering terms, a deterministic simulator can be considered to have zero \textit{variance}. 
%
...
%
How can one assess the extent to which a simulation environment is deterministic or has a permissible variance?
...
%
A good analysis of potential sources is given by Strandberg et al.~\cite{intermittently-failing-tests} although the AV simulation domain introduces it's own unique challenges that are not considered in this paper.
...
%
\todo[inline,color=red!40]{old: To ensure repeatable simulation outcomes the physics setting $dt$ must be fixed, along with any actor navigation meshes, random number seeds, game engine setup and simulation specific parameters.}
...
\bottomrule
\end{tabular}
\caption{A description of the test scenarios showing the test number, the actors included, if a collision occurred and between which actors. Where $n$ is the number of repeats and $\max\sigma$ is the maximum simulation deviation. The term \textit{unrestricted} refers to an unrestricted account of the results including results of any resource utilisation. To understand the impact of collisions and high resource utilisation the \textit{restricted} column shows a subset of the results where data post-collision has been removed and where data above a resource utilisation level of 75\% has also been removed.}
...
Practitioners should also be aware that many libraries for calculating variance itself may require attention to get precise results. For example the \texttt{numpy} method of variance is sensitive to the input precision and will return an incorrect answer if the wrong parameters are set~\cite{NumpyVar}. In \texttt{matlab}, the calculation of variance may switch from single thread to a multi-threaded execution not obviously apparent to the user when the input data size becomes large enough, opening up the potential for concurrency imprecision.\footnote{\url{https://uk.mathworks.com/matlabcentral/answers/317128-does-matlab-use-all-cores-by-default-when-running-a-program}} 
...
\begin{figure*}[t]
    ...
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{Other/Figures/CarsCollisionPrePostV4.pdf}
        \caption{}
...
\begin{figure*}[h]
    ...
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{Other/Figures/CarsPeopleCollisionPrePostV4.pdf}
        \caption{}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{Other/Figures/NICE_analysis_summary.pdf}
    \caption{Summary of investigating the effect of NICE priority setting for 0\% and 75\% CPU \& GPU Resource Utilisation.}
...
An investigation into the response of process scheduling on the simulation variance was undertaken. The experiment was repeated using test 1 but altering the process scheduling priority using the Ubuntu program \texttt{NICE}. Setting a higher priority for the simulator process with respect to the resource utilisation processes, it was possible to determine if scheduling could account for the increased simulation variance when the system is under stress.
%
...
The results presented in a box and whisker format, Fig.~\ref{NICEExperimentStressSummary}, show that decreasing the priority (right hand side of plot) has little effect on simulation variance when compared to a 0 NICE value (central plot bars). Increasing priority (left hand side of plot) reduced variance for the 75\% resource utilisation  but this did not account for all the difference in the observed results. 
%
This unaccounted difference in the variance may be due to the lack of  control that \texttt{NICE} has over the processes~\cite{Abanoub can you cite that quote you found here please}.
...
A potential avenue of exploration is developing a version of the simulator which has been designed specifically for verification. In this version the simulator would be deterministic because the execution is suitably controlled along with sources of randomness and scheduling. A similar task was achieved with the \texttt{RR} program.\footnote{\url{https://github.com/mozilla/rr}} 
