...
%
Determinism is a key requirement for simulation-based development and verification; it ensures tests are repeatable, so that software bugs can be found, fixed and fixes can be checked, and simulation results are trustworthy.
%
...
%
This paper identifies the requirements these engines need to fulfil when used for AV development and testing, and proposes a method to determine the \textit{simulation variance} for specific configurations and utilisation of the software and hardware, respectively. 
%
A case study using Carla, an open-source simulation environment powered by Unreal Engine, illustrates the proposed method and results from an example scenario indicate the presence of deterministic operational domains. \todo{Review again at end. Potentially give away more on results to motivate the reader to continue.}
...
%
Carla\footnote{\url{http://carla.org/}}, for instance, is an open-source simulator for autonomous driving that is implemented as a layer over the Unreal Engine (UE4), a  real-time 3D creation environment for the gaming and film industry as well as other creative sectors~\cite{CARLA_paper}. 
...
%
However, while game engines are designed primarily for user experience, the requirements for AV verification go beyond that and include determinism.
%
...
\todo{move to earlier :)}Determinism is a key prerequisite for simulation during AV development and testing. 
%
...
A case study to highlight the non-deterministic effects in a game engine are presented where scenarios between pedestrian and vehicle actors are used to illustrate conditions that result in high simulation variance. By limiting the the level of system utilisation to less than 75\% and terminating scenarios when any collision is detected, a deterministic operational domain was found.
%
...
This paper is structured as follows. In Section~\ref{s:background} game engines and sources of potential non-determinism in game engines are investigated. 
%
A case study to illustrate the observed simulation variance for a number of scenarios is presented in Section~\ref{s:case-study}.\todo[inline]{point to UE4 GC - ??} 
%
Section~\ref{s:methodology} presents a method to assess the suitability of a simulation system for AV verification. 
%
We conclude in Section~\ref{s:conclusion} and give an outlook on future work.\todo[inline]{review at end}
...
When the frame is rendered the game loop cycle returns to processing inputs. And a good description of the interplay between the physics and render cycles are given here~\cite{JohnAustinUnity}.
...
%
If the engine tries to render between physics updates, \textit{residual lag} can occur and extrapolation between frames needs to be performed to smooth transition between scenes. This residual lag may result in frames arriving with a delay to the simulated physics which could be result in errors for perception stack testing.
...
Any simulation hardware not using ECC memory that runs for 1000's hours, typical in AV verification, are likely to incur significant CPU hours and therefore are subject to increased exposure to these errors.
...
We present an empirical investigation to illustrate some the challenges of simulation based verification outlined in Section~\ref{s:introduction} and including the potential sources of non-determinism from Section~\ref{s:nondeterminisimSources}. The simulation must be deterministic and by our definition this should mean that the overall level of simulation variance %$\sigma^2$
