...
This paper first reviews and identifies the potential causes of non-determinisitc behaviours in game engines. 
%
This is then followed by a case study using CARLA, an open source autonomous driving simulation environment powered by Unreal Engine, which highlightes its inherent shortcomings in providing sufficient precision in experimental results.
%
Different configurations and utilisations of the software and hardware are explored to determine an opertional domain where the simulation precision is satisfactory low i.e. variance between repeats becomes negligible for development and testing work.  
...
A simulation trace is the output log from the simulator which, herein, is a time series of all actor positions ($x,y,z$) at regular time intervals. This definition could be extended to include other variables.\\%, but position is sufficient for this analysis.\\
...
Actor path variance, derived from the simulation trace, is chosen over other variables as this distance based metric forms the basis for many downstream verification tests.% such as assertion checking. 
% 
...
%Results are referred to in terms of deviation, rather than variance, as this is more intuitive to comprehend when considering verification tests, e.g. vehicle close-passing. Moreover, the maximum observed value of deviation was chosen over a mean value to ensure sufficient sensitivity.\\
