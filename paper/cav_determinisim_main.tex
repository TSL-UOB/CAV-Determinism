...
%
However, gaming engines do not need to be deterministic per se. % when used for gaming. 
%
...
Increasingly, the autonomous vehicle (AV) community is adopting gaming engines as simulation platforms to support the development and testing of vehicle control software. 
%
...
The benefit of using state-of-the-art gaming engines as simulators is that they offer sufficient realism~\cite{Koopman2018} in the physical domain combined with realistic rendering of scenes suitable for perception stack testing. 
%
...
%
Knowing the simulation variance will help developers and testers understand the impact of using a gaming engine for AV simulation, especially how potentially existing non-determinism affects repeatability and reliability.\todo{review and potentially summarize results from case study}
%
...
This paper is structured as follows. In Section~\ref{s:background} gaming engines and sources of potential non-determinism in gaming engines are investigated. Section~\ref{s:methodology} presents a method to assess the suitability of a simulation system for AV verification. 
%
A case study to illustrate how the simulation variance observed for a number of scenarios is presented in Section~\ref{s:case-study}. We conclude in Section~\ref{s:conclusion} and give an outlook on future work.\todo{review at end}
...
%     \item Necessary background on gaming engines used in AV simulations.
...
Investigating the determinism of gaming engines has not attracted much research interest since performance is more critical for playing games than accurate and repeatable runtime execution. %However, when these engines are used for AV development then deterministic behaviour is required. 
...
\begin{figure*}[b]
    ...
    \caption{Shows a flow diagram of the methodology proposed to define the simulation variance of a gaming engine for usage in AV simulations.}
...
%
AV specific platforms, such as Carla, can be operated with fixed frame rates which should provide maximum stability as long as the frame rate during execution (FPS) is monitored and observed to adhered to this rate.
%
