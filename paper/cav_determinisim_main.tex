...
7       & Two pedestrians                & Yes 		 & Pedestrian and Pedestrian & 2 		 & 1000 & 1000 \\
8       & Two pedestrians                & Yes 		 & Pedestrian and Pedestrian & 20 		 & 1000 & 1000 \\
\bottomrule
\end{tabular}
\caption{Set of experiments where $d_l$ is the \textit{look ahead distance}, $n$ is the number of repeats and $\max\sigma$ is the maximum simulation deviation.}
\label{TableOfExperiments}
\end{table*}
...
In order to reproduce conditions for computationally intensive simulations, the experiments are executed with additional levels of artificial resource utilisation. From previous work it was found that additional loads on either the CPU or GPU could increase simulation variance by equivalent levels\footnote{see footnote 14}. The level of additional resource utilisation, above that of the needs of the simulation, was increased from $0\%$ (no additional resource load) in $25\%$ increments up to a maximum of $95\%$ for both CPU and GPU as reported by system monitors (\texttt{htop} and \texttt{nvidia-smi} respectively). 

Tests 5\&6 involve only pedestrians where they do not collide (test 5) and where they do (test 6). Similar to the vehicle actors, the look ahead range ($d_l$) can be modified so that the actor is forced to navigate on waypoints that are further away. In test 7 $d_l$ is creased from $0.4m$ to $2.0m$ and in test 8 to $20m$.

Abanoub: can you change the diagram to be a staggered t-junction as the DMR is just confusing for this example. Also can we zoom in on the lower right area. Tests 5 and 6 have different look ahead values, which is correct?
...
The list of tests shown in Table \ref{TableOfExperiments} were chosen to cover a range of interactions between actor types. Test 1 \& 2 involve 2 vehicles meeting at a junction where they do not collide (test 1) and when they do tiggering a collision callback in the game engine (test 2). In both cases the trajectories of the vehicles are hard-coded to follow a set of waypoints spaced at 0.1m intervals. Vehicle actors were programmed to use a PID controller to follow the waypoints targetting the closest waypoint but ignoring any waypoints closer than a fixed \textit{look-ahead distance} ($d_l$)\footnote{Targeting waypoints that are within $d_l$ tend to cause improper navigation with the implemented PID controller}.

In test 3 two vehicles dive without collision and a pedestrian walks across the road at a crossing point. In test 4 this pedestrian collides with one of the vehicles at the crossing, see Fig~\ref{Test_a}. Pedestrians navigate to a set of regularly space waypoints at 0.1m intervals using the A* algorithm which are the default settings in the Caral/Unreal distribution.
