...
\todo[inline]{Determinism is a key requirement for simulation-based development and verification; it ensures the simulation is precise and that tests are repeatable, so that software bugs can be found and fixed, and simulation results are trustworthy.}
%
...
%
A case study using CARLA, an open-source simulation environment powered by Unreal Engine, illustrates the proposed method and results from an example scenario indicate the presence of a deterministic operational simulation setup. \todo{Review again at end. Potentially give away more on results to motivate the reader to continue.}

\todo[inline]{A case study using CARLA, an open-source simulation environment powered by Unreal Engine, illustrates the proposed method and results from an example scenario demonstrate simulation precision to an acceptable tolerance. }
\todo{Review again at end. Potentially give away more on results to motivate the reader to continue.}
...
%
\todo{GC Replace: However, several challenges remain; one of these is determinism. with:}
%
...
%
If the simulation is not deterministic or has a non-permissible variance in the tolerance of, for example, actor positions, this may lead to assessment errors, making it difficult to understand and remove bugs. In the worst case, bugs that could have been identified in simulation remain undetected, resulting in false confidence in the safety of the AV's control software. The term \texit{simulation variance} refers to a measure of tolerance in the simulation and actor position is used as an example throughout this paper.
...
/todo{GC: How can one assess the extent to which a simulation environment is deterministic or has permissible tolerance?} 
...
%
In our case study, scenarios between pedestrian and vehicle actors have been analysed to identify conditions that result in non-deterministic simulation traces as a result of non-permissible actor position variance.
%
...
\todo[inline]{GC Edit: Considering the objectives for gaming and comparing them to these for AV development and testing, there are fundamental differences. Providing game players with a responsive real-time experience is often achieved at the cost of simulation accuracy and precision. The gamer neither wishes a faithful representation of reality, i.e. can be low accuracy, nor do they require for repeated actions to result in the same outcome to particular tolerance, i.e. can be low precision. In contrast, the precision required for AV development and testing is much higher, especially for perception stack testing, but also for visual inspection of accidents or near misses. If necessary, it is acceptable to achieve the required level of tolerance on the precision at the cost of real-time performance. For example, for perception stack testing the sensors need to get input that is as close as possible to what the real input would be, i.e.\ with no or minimal extrapolation or lag. This  may only be realisable by slowing down the rendering to enable more extensive physics calculations.}

\todo{KIE: ... by slowing down the simulation to enable more extensive physics calculations?}
...
\todo[inline]{Check the points on floating point operations in An Empirical Analysis of Flaky Tests~\cite{empirical-analysis-of-flaky-tests}.}
...
%
Memory checking not withstanding, errors associated with the lack of ECC are likely to be minimal unless there is significant background radiation or 1000's of hours of computation are expected.\todo{So, do commercial HPC facilities have ECC memory?}
%
...
\todo[inline]{To ensure precise simulation outcomes the physics setting $dt$ must be fixed, along with any actor navigation meshes, random number seeds, game engine setup and simulation specific parameters.}
%
...
%
\todo{Complete the section structure as this is a very long section.}
...
% ***************************************************
\subsection{Context, Scene and Scenario}\label{TestsDescriptionAndTechnicalities}\todo{Need to stick to scene, scenario definitions as in Intro section.}
...
\todo{Decide past or present tense and be consistent throughout.}
...
\todo{Set the threshold, explain what a good threshold is, available options and what was selected and why.}
