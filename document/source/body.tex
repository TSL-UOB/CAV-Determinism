...
avoiding floating point representation entirely~\cite{empirical-analysis-of-flaky-tests}, we argue that the precision issues related to floating-point operations are better described as \textit{incorrectness} that is in fact repeatable; they do not cause non-determinism per se. So, even if the result of a mathematical operation is incorrect due to floating-point rounding errors, it should always be equally \textit{incorrect} when repeated for implementations that conform to the IEEE floating-point standard~\cite{8766229}. 
%
...
It is responsibility of the tester
% It is unlikely 
It's unlikely that changes to thread priorities or scheduling policy would occur during repeated tests on the same hardware and should therefore give the same output. However, if some aspects of the game loop are multi-threaded\footnote{\url{https://docs.unity3d.com/Manual/JobSystemMultithreading.html}} 
...
The Unreal documentation and source code suggests that CARLA uses a priority queue based on a binary heap 
...
Theoretically this means that use of A* in CARLA will be determinstic as long as the heap is always constructed in the same way.
However, if some unknown change causes the heap to be constructed in a different way, then the use of A* may then appear non-deterministic.
...
Performing physics calculations and running the simulator may consume a significant proportion of system resources (e.g. CPU and GPU processors) for higher performing simulations. An initial hypothesis was that as this ratio tended toward one, there would be an increase in \textit{simulation variance} which was corroborated with some initial investigations~\cite{TSLUnrealEngineTesting} and explored more fully in this work using the Carla platform. 
...
%
In particular, actor collisions and system-level resource utilisation were identified as key contributors to increased simulation variance when using Carla and we recommend monitoring these during simulation execution or using a commercial alternative, e.g.~\cite{rFpro2021}. 
%
...
%
% Alternatively, there are commercially available driving simulators that claim to be fully deterministic, e.g.\ RFPro~\cite{rFpro2021}, that may be more suitable if using a game engine does not provide a simulation variance sufficient for the verification requirements.
