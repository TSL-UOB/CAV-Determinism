...
It is unlikely that changes to thread priorities or scheduling policy would occur during repeated tests on the same hardware and should give the same output. However, if some aspects of the game loop are multi-threaded~\cite{unity_multithreading} or if the scheduler simply randomly selects from a set of threads with equal priority, this may alter an otherwise deterministic sequence of events. 
...
The Unreal documentation and source code suggests that CARLA uses a priority queue based on a binary heap~\cite{FGraphAStar,TArray,GraphAStar}.
