...
To summarise, documentation suggests that Carla uses a binary heaps for the priority queue~\cite{FGraphAStar,TArray,GraphAStar} which preserves some order of the frontier nodes, but not necessarily the insertion order, and hence is unstable. Therefore, determinism of the A* algorithm for actor navigation will be sensitive to the ordering preserved by the priority queue, in this case the heap order. This also assumes the simulation environment itself is deterministic and sources of non-determinism that affect the simulator or the system more generally may cause changes to the operation of an A* implementation.
