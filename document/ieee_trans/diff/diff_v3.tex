...
\begin{figure}[!t]
    ...
    \begin{subfigure}{.48\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/Variance_predicition_tolerance_definition_diagram_a.pdf}
        \caption{Non-determinism}
        \label{variance_description_a}
    \end{subfigure}

    \begin{subfigure}{.48\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/Variance_predicition_tolerance_definition_diagram_b.pdf}
        \caption{Determinism}
...
.}\DIFdelend \DIFaddbegin \DIFadd{, Unity}\footnote{\url{https://unity.com/}} \DIFadd{and CryEngine.}\footnote{\url{https://www.cryengine.com/}} 
...
\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{\pathToOtherFiles/figures/GameEngineLoopv2.pdf}
...
%DIF < 
Floating-point representation of real numbers }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend is limited by the fixed bit width available \DIFdelbegin \DIFdel{in the hardware, resulting in the finite precision with which numbers can be represented in a computation. 
Thus, the results of arithmetic operations must fit into the given bit width, which is achieved through rounding to the nearest representable value. This gives rise to }\DIFdelend \DIFaddbegin \DIFadd{and can potentially result }\DIFaddend rounding errors~\cite{FloatingPointsBook,goldberg1991every}. %DIF < 
...
%DIF < 
}\DIFdelend So, even if the result of a \DIFdelbegin \DIFdel{floating-point }\DIFdelend \DIFaddbegin \DIFadd{mathematical }\DIFaddend operation is incorrect due to floating-point rounding errors, it should always be equally \textit{incorrect} \DIFaddbegin \DIFadd{when repeated }\DIFaddend for implementations that conform to the IEEE floating-point standard~\cite{8766229}. 
\DIFdelbegin %DIFDELCMD < 
...
%DIF < 
Furthermore, floating-point calculation results may differ }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend if the execution is performed on a GPU rather than a CPU\DIFdelbegin \DIFdel{which may have different register widths~\mbox{%DIFAUXCMD
\cite{Whitehead2011}}\hspace{0pt}%DIFAUXCMD
. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \footnote{ \DIFadd{CPU and GPU processors may have different register widths~\mbox{%DIFAUXCMD
\cite{Whitehead2011} }\hspace{0pt}%DIFAUXCMD
}} \DIFadd{may all affect the execution order. 
%DIF > 
...
%DIF < 
However, scheduling }\DIFdelend \DIFaddbegin \DIFadd{Scheduling }\DIFaddend multiple processes across several processing cores, where the number of cores is smaller than the number of processes, \DIFaddbegin \DIFadd{or executing across a parallel GPU architecture, }\DIFaddend can result in variation of the execution order and cause simulation variance unless explicitly constrained or statically allocated prior to execution. \DIFdelbegin %DIFDELCMD < 
...
\begin{figure}[!t]
    ...
    \begin{subfigure}{.24\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/fig3a}
        \caption{}
        \label{Test_a}
    \end{subfigure}
    \begin{subfigure}{.24\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/det_fig3bb}
        \caption{}
        \label{Test_b}
    \end{subfigure}
    \begin{subfigure}{.24\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/det_fig3c}
        \caption{}
        \label{Test_c}
    \end{subfigure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{\pathToOtherFiles/figures/actor_path_plot.jpg}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{\pathToOtherFiles/figures/ExperimentsStressSummaryV6.pdf}
...
\DIFdelend \begin{figure*}[t]
    ...
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/CarsCollisionCG25_V4.pdf}
        \caption{}
        \label{CarsCollisionCG25}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/CarsCollisionPrePostV5.pdf}
        \caption{}
...
\begin{figure*}[h]
    \centering
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/CarsPeopleCollsionsCG25_V3.pdf}
        \caption{}
        \label{CarsPeopleCollsionsCG25}
    \end{subfigure}
    \begin{subfigure}{.49\textwidth}
        \includegraphics[width=1\textwidth]{\pathToOtherFiles/figures/CarsPeopleCollisionPrePostV5.pdf}
        \caption{}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.48\textwidth]{\pathToOtherFiles/figures/NICE_analysis_summary_V4.pdf}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.99\linewidth]{\pathToOtherFiles/figures/Methodology_Diagram_v8.pdf}
