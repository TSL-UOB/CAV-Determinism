...
namespace
{
	...
	void CreateUnrealMaterial(FString sourceFilePath, FString packagePath, FRoadRunnerImportUtil::MaterialInfo materialInfo, int metadataVersion,TMap<FString, int32>& materialToLayerMap)
	{
		...
		FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("DiffuseMap")), sourceFilePath, materialInfo.DiffuseMap, packagePath, false);
		FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("NormalMap")), sourceFilePath, materialInfo.NormalMap, packagePath, true);
		FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("RoughnessMap")), sourceFilePath, materialInfo.SpecularMap, packagePath, false);
...
void FRoadRunnerImporterModule::RoadRunnerPostProcessing(UFactory* inFactory, UObject* inCreateObject)
{
	...
	if (roadrunnerFactory->RoadRunnerImportOptions->OverwriteMaterials)
	{
		...
		{
			CreateUnrealMaterial(srcPath, packagePath, matInfo, metadataVersion, materialToLayerMap);
		}
