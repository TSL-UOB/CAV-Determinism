...
namespace
{
	...
	void CreateCarlaMaterial(FString sourceFilePath, FString packagePath, FRoadRunnerImportUtil::MaterialInfo materialInfo, int metadataVersion)
	{
		...
		baseMaterial = FindExistingUnrealMaterial(materialPath, materialName);
#else
		...
		{
			FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("basecolorA")), sourceFilePath, materialInfo.DiffuseMap, packagePath, false);
			FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("NormalA")), sourceFilePath, materialInfo.NormalMap, packagePath, true);
			FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("RoughnessA")), sourceFilePath, materialInfo.SpecularMap, packagePath, false);
		...
		{
			FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("BaseColor")), sourceFilePath, materialInfo.DiffuseMap, packagePath, false);
			FRoadRunnerImportUtil::SetTextureParameter(unrealMaterial, FName(TEXT("None")), sourceFilePath, materialInfo.NormalMap, packagePath, true);
			...
			if (assetObject->IsA(UStaticMesh::StaticClass()))
			{
				...
				if (rrFactory->RoadRunnerImportOptions->OverwriteMaterials)
				{
					...
					for (auto& matref : staticMesh->StaticMaterials)
					{
						...
						materialFullName = FRoadRunnerImportUtil::MakeName(materialFullName);
